...
- `Evidência direta`: `scripts/Test-GeneXusXpzImportPreview.ps1` e `scripts/Invoke-GeneXusXpzImport.ps1` gravam `msbuild.import.signals.json` ao lado dos logs brutos quando `Read-MsBuildImportSignals.ps1` consegue executar; falha nessa gravacao degrada o diagnostico (`diagnosticDegraded`), mas nao reclassifica a task Import como falha operacional quando `executionEvidence.msBuildExitCode` e stdout sustentam conclusao.
