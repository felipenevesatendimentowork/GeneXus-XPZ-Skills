...
- `Evidência direta`: `scripts/GeneXusKbDeploymentEnvironmentSupport.ps1`, `scripts/GeneXusKbEnvironmentInventorySupport.ps1`, `scripts/Get-GeneXusKbEnvironmentNames.ps1` e `scripts/Set-XpzKbSourceMetadataDeployment.ps1` leem/gravam `deployment_environment_name`, `deployment_hosting_kind`, `kb_environment_count` e `kb_environment_names` em `kb-source-metadata.md` (inventario MSBuild obrigatorio no setup via `-InventoryFromGeneXusMsBuild`; heuristica `-InventoryFromKbNativePath` por pastas `web\` removida); `Invoke-GeneXusKbBuildAll.ps1` e `Invoke-GeneXusKbSpecifyGenerate.ps1` bloqueiam (exit 46, causa `environment-validacao-indefinido`) quando KB multi-environment sem environment de validacao resolvido; JSON inclui `deploymentEnvironmentContext` e warning se `ActiveEnvironment` divergir do environment de validacao.
