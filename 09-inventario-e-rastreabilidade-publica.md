...
- `Evidência direta`: `scripts/Query-KbIntelligenceIndex.py` bloqueia consultas semanticas (`who-uses`, `what-uses`, `impact-basic`, `functional-trace-basic`) quando `queryableByKbIntelligence=false` no catalogo (`blocked`, `reason=QUERY_NOT_SEMANTIC_FOR_TYPE`, exit `11`); tipos com flag `false` na base compartilhada incluem `SmartDevicesPlus`, `DataStore`, `Generator`, `ThemeClass`, `ThemeColor`, `Folder`, `PatternSettings` e `RootModule` (virtual); bateria `scripts/Test-KbIntelligenceQueryableGuardSelfTest.ps1`; amostra de envelope XML multi-KB em `scripts/Invoke-ParallelKbEnvelopeScan.ps1`.
