...
- `Evidência direta`: `scripts/GeneXusKbDeploymentEnvironmentSupport.ps1` e `scripts/Set-XpzKbSourceMetadataDeployment.ps1` leem/gravam `deployment_environment_name`, `deployment_hosting_kind`, `kb_environment_count` e `kb_environment_names` em `kb-source-metadata.md` (inventario na KB nativa somente no setup); `Invoke-GeneXusKbBuildAll.ps1` e `Invoke-GeneXusKbSpecifyGenerate.ps1` bloqueiam (exit 46, causa `environment-validacao-indefinido`) quando KB multi-environment sem environment de validacao resolvido; JSON inclui `deploymentEnvironmentContext` e warning se `ActiveEnvironment` divergir do environment de validacao.
- `Evidência direta`: `scripts/GeneXusKbDeployBinSupport.ps1` e `scripts/Test-GeneXusDeployBinFreshness.ps1` checam frescor de `web\bin` do environment de deploy pos-build; `-PostImportDeployValidation` / `-StrictDeployBinCheck` ativam gate (exit **49**); status `compilou-mas-dll-destino-desatualizada`; campos `deployBinFreshness` e `deployBinCheck` no JSON dos wrappers de build.
