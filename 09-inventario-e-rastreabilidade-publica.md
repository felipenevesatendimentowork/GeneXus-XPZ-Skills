...
- `Evidência direta`: `scripts/Read-MsBuildImportSignals.ps1` com `-Stage export` classifica linhas `error :` do stdout/stderr em `exportErrors`, tipos rejeitados em `invalidTypesRejected` e ruido conhecido em `knownStdOutNoise`; `scripts/Invoke-GeneXusXpzExport.ps1` grava `msbuild.export.signals.json`, promove `exportErrors`, `invalidTypesRejected` e `knownStdOutNoise` ao top-level do `export.json` (alem de `stdoutSignals`/`compactSignals`) e pode definir `operationalSubState=exportação concluída com erros parciais ignorados pela task` quando a task Export conclui com sucesso aparente mas o log contem erros parciais.
...
- `Evidência direta`: `scripts/GeneXusPackageInventorySupport.ps1` com `scripts/Build-GeneXusImportFileEnvelope.ps1` e `scripts/New-XpzImportPackage.ps1` embute `packageInventory` no JSON de retorno apos gravar o pacote; `scripts/Test-GeneXusPackageInventorySupportSelfTest.ps1` valida `nominalInventoryAt`, `extrasFullListAt` e `attributesTopLevelUnreconciled` em pacote sintetico; sentinela `GENEXUS_PACKAGE_INVENTORY_SUPPORT_SELFTEST_OK`.
