...
- `Evidência direta`: `scripts/GeneXusKbDeploymentEnvironmentSupport.ps1`, `scripts/GeneXusKbEnvironmentInventorySupport.ps1`, `scripts/Get-GeneXusKbEnvironmentNames.ps1`, `scripts/Set-XpzKbSourceMetadataDeployment.ps1` e `scripts/Resolve-GeneXusGeneratedCsPath.ps1` leem/gravam/consomem `deployment_environment_name`, `deployment_hosting_kind`, `kb_environment_count`, `kb_environment_names`, `kb_environment_output_dirs` e `kb_environment_web_dirs` em `kb-source-metadata.md` (setup: `-KbEnvironmentNames` e `-KbEnvironmentOutputDirs` declarados pelo usuario + validacao MSBuild obrigatoria via `SetActiveEnvironment` headless; scan automatico por pastas removido); inventario validado emite `KB_ENVIRONMENT_INVENTORY_OK`; `Invoke-GeneXusKbBuildAll.ps1` e `Invoke-GeneXusKbSpecifyGenerate.ps1` bloqueiam (exit 46, causa `environment-validacao-indefinido`) quando KB multi-environment sem environment de validacao resolvido; JSON inclui `deploymentEnvironmentContext` e warning se `ActiveEnvironment` divergir do environment de validacao; resolvedor de `.cs` bloqueia quando `kb_environment_web_dirs` nao cobre o environment.
