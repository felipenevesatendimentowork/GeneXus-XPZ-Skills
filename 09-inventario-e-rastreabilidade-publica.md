...
- `Evidência direta`: `scripts/GeneXusExportTaskLabelSupport.ps1`, `scripts/Build-ExportTaskLabelCoverageMap.ps1`, `scripts/Run-ExportTaskLabelMatrix.ps1`, `scripts/Invoke-ExportTaskLabelCampaign.ps1` e `scripts/Merge-ExportTaskLabelCampaignResults.ps1` implementam a campanha operacional de matriz `exportTaskLabel` (cobertura, execução MSBuild por tipo, consolidação e opcional `-ApplyCatalog`); evidência de rodada em `historico/export-task-label-matrix-20260530/` e contrato em `10a-gx-export-task-labels.md`.
