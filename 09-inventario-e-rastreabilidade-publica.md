...
- `Evidência direta`: `scripts/msbuild-exit-codes.catalog.json` — índice versionado de `exitCode` dos wrappers MSBuild (probe `10`–`16`, import/export `31`–`35`/`41`–`42`, politica **46** com `causes[]`, cancelamento **47**, Categoria B **48**, build **40**–**45**, deploy bin freshness **49**, `-LogPath` diretorio existente **50** (`parametro-invalido`, gate fail-fast, motor `scripts/GeneXusMsBuildLogPathSupport.ps1`), falha interna **90**); exit **35** = pré-validação de identidade em lista seletiva (`objectListPreflight`); exit **46** inclui, entre outras causas, `ForceRebuild=true` sem `-AllowWideRebuild`, `CompileMains=true`/`DetailedNavigation=true` sem `-AllowCostlyBuildOptions` e confirmações desacopladas de seus switches; verificacao `scripts/Test-MsBuildExitCodesCatalog.ps1` (sentinela `MSBUILD_EXIT_CODES_CATALOG_OK`).
- `Evidência direta`: `scripts/GeneXusMsBuildLogPathSupport.ps1` é o motor puro compartilhado do gate fail-fast de `-LogPath` dos wrappers MSBuild: `Get-GeneXusMsBuildLogPathRejection` rejeita quando o `-LogPath` resolvido aponta para um **diretório existente** (`Test-Path -PathType Container`, nunca `-not -PathType Leaf`, para não bloquear arquivo-a-criar com pai válido) e `New-GeneXusMsBuildLogPathBlockJson` monta o bloco `exitCode=50` (`parametro-invalido`). É função pura: não grava nem resolve caminho (recebe o `-LogPath` já resolvido pelo `Get-FullPathSafe` local de cada wrapper). Os 10 wrappers que recebem `-LogPath` — `Invoke-GeneXusKbBuildAll.ps1`, `Invoke-GeneXusXpzImport.ps1`, `Get-GeneXusKbProperty.ps1`, `Start-GeneXusKbBuildDetached.ps1`, `Invoke-GeneXusXpzImportThenBuild.ps1`, `Invoke-GeneXusKbSpecifyGenerate.ps1`, `Invoke-GeneXusXpzExport.ps1`, `Test-GeneXusXpzImportPreview.ps1`, `Open-GeneXusKbHeadless.ps1` e `Test-GeneXusKbConsistency.ps1` — chamam o gate cedo (antes de abrir a KB / registrar tarefa) e, quando rejeitado, emitem o bloco **só no stdout** (sem `Write-JsonLog`, para não reintroduzir a gravação no `-LogPath` inválido) e retornam `exit 50`; sem o gate, um diretório passado como `-LogPath` faria a operação concluir e só falhar na gravação final, mascarando o resultado como `exit 90` («falha operacional»). `scripts/Test-GeneXusMsBuildLogPathSupportSelfTest.ps1` valida diretório-rejeitado, arquivo-a-criar, arquivo existente, vazio/whitespace/null e o contrato do bloco JSON (sentinela `GENEXUS_MSBUILD_LOGPATH_SUPPORT_SELFTEST_OK`); o `wrapperScanList` de `scripts/Test-MsBuildExitCodesCatalog.ps1` foi ampliado com `Start-GeneXusKbBuildDetached.ps1` e `Invoke-GeneXusXpzImportThenBuild.ps1` para cobrir o `exit 50` deles.
...
- `Evidência direta`: `Sync-GeneXusXpzToXml.ps1` reconcilia o acervo por `guid` do nó raiz em `-FullSnapshot` (`Resolve-GuidAwareRenames`): GUID igual sob nome diferente é tratado como rename — renomeia o arquivo no acervo (`Move-Item` antigo → novo). Se o nome novo já existe com o **mesmo** GUID, remove o órfão; se existe com GUID **diferente** (colisão de nome), **aborta fail-closed** antes do laço de escrita para preservar o objeto existente; rename só de caixa é efetivado via etapa intermediária (NTFS). Cobre `Attribute` e `Object`; reporta `RenamedByGuid`/`RenameResidualsDetected` no summary e `Renames` no relatório. Em `-VerifyOnly`, classifica o resíduo sem tocar o disco. `scripts/Test-XpzSyncGuidRenameSelfTest.ps1` valida 6 cenários (rename real, conferência `-VerifyOnly`, zero-rename, órfão de mesmo GUID, colisão de GUID fail-closed e rename só de caixa); sentinela `OK: Test-XpzSyncGuidRenameSelfTest.ps1`.
...
- `Evidência direta`: `scripts/Test-XpzWrapperInventory.ps1` passou a emitir `INVENTORY_LEGACY_ORPHANS` quando scripts legados permanecem lado a lado com nomes canonicos atuais e `INVENTORY_RECOMMENDED_MISSING` quando sinais objetivos de uso recomendam wrappers finos ainda ausentes; para `Set-KbSourceMetadataDeployment.example.ps1`, detecta wrapper local que ainda repassa descoberta automática de environments (`InventoryFromKbNativePath`/`InventoryFromGeneXusMsBuild`) ou omite `-KbEnvironmentNames`, `-KbEnvironmentOutputDirs` e parâmetros de validação MSBuild (`KbNativePath`, `InventoryWorkingDirectory`); para os wrappers K8/K9 `Test-KbSetupAudit.example.ps1` e `Test-KbIndexGate.example.ps1`, emite `INVENTORY_CUSTOMIZED` com motivo `missing_AsJson_passthrough` quando o molde repassa `-AsJson` ao motor mas o wrapper local não o repassa, fechando a divergencia em que tal wrapper passaria na auditoria de setup mas bloquearia a rotina pré-push de pasta paralela (gates K8/K9 da skill `xpz-kb-parallel-pre-push`).
