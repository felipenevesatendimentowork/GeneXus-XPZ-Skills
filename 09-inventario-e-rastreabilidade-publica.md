...
- `Evidência direta`: `scripts/Find-CsAttributeAssignments.ps1` e `scripts/GeneXusCsAttributeAssignmentSupport.ps1` mapeiam atribuicoes de atributo Transaction no `.cs` gerado (camada web), `AssignAttri` por metodo e padrao triplet do Specifier (`cascadeOrder`); verificacao `scripts/Test-FindCsAttributeAssignmentsContract.ps1` (sentinela `FIND_CS_ATTRIBUTE_ASSIGNMENTS_CONTRACT_OK`); consumo complementar pos-build em `xpz-msbuild-build/SKILL.md`.
