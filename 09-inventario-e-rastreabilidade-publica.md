...
- `Evidência direta`: `scripts/Repair-PtBrAccentDegradation.ps1` é a contraparte aplicadora do medidor — corrige só as ocorrências inequívocas (palavras da lista curada cuja forma sem acento é sempre erro) nos `.md` indicados, reusando lista, regex e supressão de código do detector via dot-source; preserva caixa, EOL LF e UTF-8 sem BOM, e respeita a fronteira pt-BR (`Get-PtBrLineCount`) para não tocar seções `## Español`/`## English`. Tokens ambíguos e cópula geral ficam fora por construção (decisão humana). Usado na frente de correção de acentuação registrada no `999-ideias-pendentes.md`.
