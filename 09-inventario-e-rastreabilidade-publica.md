...
- `Evidência direta`: `scripts/GeneXusKbDeploymentEnvironmentSupport.ps1`, `scripts/GeneXusKbEnvironmentInventorySupport.ps1`, `scripts/Get-GeneXusKbEnvironmentNames.ps1` e `scripts/Set-XpzKbSourceMetadataDeployment.ps1` leem/gravam `deployment_environment_name`, `deployment_hosting_kind`, `kb_environment_count` e `kb_environment_names` em `kb-source-metadata.md` (setup: `-KbEnvironmentNames` declarado pelo usuario + validacao MSBuild obrigatoria via `SetActiveEnvironment` headless; scan automatico por pastas removido); inventario validado emite `KB_ENVIRONMENT_INVENTORY_OK`; `Invoke-GeneXusKbBuildAll.ps1` e `Invoke-GeneXusKbSpecifyGenerate.ps1` bloqueiam (exit 46, causa `environment-validacao-indefinido`) quando KB multi-environment sem environment de validacao resolvido; JSON inclui `deploymentEnvironmentContext` e warning se `ActiveEnvironment` divergir do environment de validacao.
- `Evidência direta`: `scripts/Test-XpzKbMetadataWrapper.ps1` compara o output do wrapper local `Test-*KbMetadataWrapper.ps1` (em `xpz-kb-parallel-setup`) com `kb-source-metadata.md`; valida campos obrigatorios (`last_xpz_materialization_run_at`, `kb_name`, `source_guid`) e campos opcionais de environment/deploy (`deployment_environment_name`, `deployment_hosting_kind`, `kb_environment_count`, `kb_environment_names`) quando presentes no metadata; classifica `OK`, `PENDENTE_DE_DADOS`, `PENDENTE` ou `BLOCK`, consumido por `scripts/Test-XpzSetupAudit.ps1` para alimentar as linhas `metadata wrapper:` e `metadata/deploy:` do relatorio de auditoria.
- `Evidência direta`: `scripts/GeneXusKbDeployBinSupport.ps1` e `scripts/Test-GeneXusDeployBinFreshness.ps1` checam publicacao em `web\bin` do environment de deploy pos-build (max DLL de objeto excluindo runtime GeneXus/System/Microsoft, ou `*.config`; `GxNetCoreStartup.dll` so complementar em Core); `-PostImportDeployValidation` / `-StrictDeployBinCheck` ativam gate (exit **49**); status `compilou-mas-dll-destino-desatualizada`; campos `deployBinFreshness`, `deployBinCheck`, `publicationFreshSinceBuild`, `objectDllMaxWriteTime` e `configMaxWriteTime` no JSON dos wrappers de build.
