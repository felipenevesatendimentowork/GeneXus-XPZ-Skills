...
- `Evidência direta`: `Sync-GeneXusXpzToXml.ps1` reconcilia o acervo por `guid` do nó raiz em `-FullSnapshot` (`Resolve-GuidAwareRenames`): GUID igual sob nome diferente é tratado como rename — renomeia o arquivo no acervo (`Move-Item` antigo → novo) ou remove órfão de mesmo GUID quando o alvo já existe, cobrindo `Attribute` e `Object`; reporta `RenamedByGuid` no summary e `Renames` no relatório. Em `-VerifyOnly`, classifica o resíduo sem tocar o disco (`RenameResidualsDetected`). `scripts/Test-XpzSyncGuidRenameSelfTest.ps1` valida os dois caminhos (rename real em `-FullSnapshot` e classificação sem mover em `-VerifyOnly`); sentinela `OK: Test-XpzSyncGuidRenameSelfTest.ps1`.
