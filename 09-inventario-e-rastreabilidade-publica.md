...
- `Evidência direta`: `scripts/GeneXusMsBuildPostBuildEventsSupport.ps1` centraliza a extração de `postBuildEvents` em stdout MSBuild, preferindo a janela iniciada por `Executando eventos pos-construcao ...` até o próximo separador `==========` e mantendo fallback histórico para `start c:` / `start cmd`; consumido por `scripts/Invoke-GeneXusKbBuildAll.ps1` e `scripts/Invoke-GeneXusKbSpecifyGenerate.ps1` para preencher `stdoutSignals.postBuildEvents` e impedir classificação limpa quando processos externos pós-build foram disparados ou tentados; `scripts/Test-GeneXusMsBuildPostBuildEventsSupportSelfTest.ps1` cobre `start`, `call`, `cmd /k`, `powershell`, `REM`, janela vazia e fallback legado (sentinela `GENEXUS_MSBUILD_POST_BUILD_EVENTS_SUPPORT_SELFTEST_OK`).
