...
- `Evidência direta`: `scripts/Read-MsBuildImportSignals.ps1` com `-Stage export` classifica linhas `error :` do stdout/stderr em `exportErrors`, tipos rejeitados em `invalidTypesRejected` e ruido conhecido em `knownStdOutNoise`; `scripts/GeneXusMsBuildCategoryBSupport.ps1` e `scripts/msbuild-exit-codes.catalog.json` (exit **48** = Categoria B); `scripts/Invoke-GeneXusXpzExport.ps1`, `Invoke-GeneXusXpzImport.ps1`, `Test-GeneXusXpzImportPreview.ps1`, `Invoke-GeneXusKbBuildAll.ps1` e `Invoke-GeneXusKbSpecifyGenerate.ps1` rebaixam `exitCode` para 48 quando `msBuildExitCode=0` mas o log traz rejeicao B; `operationalSubState` de export: `exportação parcial com errors do MSBuild — artefato não confiável`; self-tests `Test-GeneXusMsBuildCategoryBSupportSelfTest.ps1`, `Test-GeneXusXpzExportErrorBarringSelfTest.ps1`.
