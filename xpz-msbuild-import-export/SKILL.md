...
- Quando `export.json` trouxer `packageInventory`, reproduzir no relatório ao usuário **antes** de declarar o XPZ como conclusão limpa: totais reais, `objectsByType`, `systemModulesPresent`, `systemExternalObjectsPresent`, `attributesTopLevelUnreconciled` / `inventoryWarnings` quando existirem, e em export seletiva `extrasCount` / amostra de extras — não basta o campo existir só no JSON; quando `exportErrors`, `invalidTypesRejected` ou `knownStdOutNoise` (top-level) estiverem presentes, reproduzi-los também no texto ao usuário
