...
- Perguntas do usuário em turnos posteriores da **mesma rodada** sobre o conteúdo de um XPZ já gerado («por que X está aí?», «tem Y nesse pacote?», «quais atributos exatamente?», «quais Domains?») exigem reler `package-inventory.json` da rodada (via `packageInventory.nominalInventoryAt`, `packageInventory.packageInventoryPath` ou `artifacts.PackageInventoryPath` no `export.json`) antes de responder, em vez de responder pela memória do turno anterior; `extrasSample`, `objectsByType` e contagens do `export.json` são resumo amostral ou agregado; a fonte autoritativa do conteúdo nominal é `package-inventory.json` (ou nova execução de `Get-GeneXusImportPackageObjectInventory.ps1` sobre o `.xpz`)
...
- `Get-GeneXusImportPackageObjectInventory.ps1`: inventário determinístico de `import_file.xml`, XML com raiz `<ExportFile>` ou `.xpz` (XML interno único); lista `<Object>` sob `<Objects>`, `Attribute` top-level sob `<Attributes>`, agrega `objectsByType`, detecta módulos de plataforma via `scripts/gx-system-modules.txt`, ExternalObjects de plataforma via `scripts/gx-system-external-objects.txt`, sinal `attributes-top-level-em-export-cirurgico` em export seletiva sem `Transaction` na lista declarada, e confronta delta declarado (`-DeclaredDeltaPath` ou `-DeclaredDeltaItems` no formato `Tipo:Nome`, separador `;` ou linha)
- `Invoke-GeneXusXpzExport.ps1`: após exportação com XPZ gerado, embute `packageInventory` resumido no diagnóstico e grava sempre `package-inventory.json` completo no diretório de artefatos da rodada; o resumo expõe `nominalInventoryAt` (lista nominal completa no sidecar), `extrasSample` (extras de `<Objects>` quando `extrasCount ≤ 50`), `extrasSampleTruncated` + `extrasFullListAt` quando `extrasCount > 50`; atributos top-level **não** entram em `extrasSample`; falha de inventário marca `inventoryDegraded=true` sem rebaixar `exitCode` da task MSBuild
...
- **Checklist obrigatório** antes de **importação real** quando o pacote **não** foi montado na mesma rodada pelo fluxo `xpz-builder` com manifesto explícito na conversa (objetos + intenção do lote):
  ...
  - Se aparecer **módulo de sistema / plataforma** GeneXus (por exemplo `PackagedModule:GeneXus` em export real, ou `Module:` com nome do catálogo em `gx-system-modules.txt`) ou **ExternalObject de plataforma** (nome em `gx-system-external-objects.txt`) num pacote tratado como delta mínimo → **ABORT** salvo pedido explícito desse conteúdo.
...
- **Módulos de sistema:** objeto sob `<Objects>` com `name` em `scripts/gx-system-modules.txt` e tipo `PackagedModule` (export real GeneXus 18 para SDK/plataforma, GUID `c88fffcd-b6f8-0000-8fec-00b5497e2117`) ou `Module` (container de KB do usuário, GUID `00000000-0000-0000-0000-000000000006`) aparece em `systemModulesPresent`. Exemplo operacional: `PackagedModule:GeneXus`, não apenas `Module:GeneXus`. Num pacote cirúrgico, presença de módulo de sistema não pedido exige declaração explícita ao usuário antes de preview/import/sync — mesmo que o MSBuild tenha concluído com `exitCode=0`.
- **ExternalObjects de plataforma:** objeto `ExternalObject` (GUID `c163e562-42c6-4158-ad83-5b21a14cf30e`) com `name` em `scripts/gx-system-external-objects.txt` aparece em `systemExternalObjectsPresent` — mesmo tratamento operacional que módulos de sistema em pacote cirúrgico.
...
- **Anti-padrão (nomeado): amostra de extras no lugar da lista completa** — relatar `extrasSample` ou `totalAttributes` como se fossem o conteúdo real do pacote, omitir atributos top-level quando `attributesTopLevelUnreconciled=true`, ou ignorar `package-inventory.json` quando o motor já gravou `nominalInventoryAt` / `packageInventoryPath` / `artifacts.PackageInventoryPath`. Em export seletiva com `extrasCount > 0` ou atributos top-level relevantes, a comunicação ao usuário deve listar **cada item por nome**, separado por bloco (`<Objects>` e `<Attributes>`). O JSON resumido serve à máquina; não substitui a comunicação ao humano.
- **Erros parciais no stdout (`error :`) com sucesso aparente da task:** a task `Export` pode emitir linhas `error :` (ex.: `X is not a valid type`) e ainda concluir com `Export Sucesso` e `__EXPORTED_FILE__`. O wrapper captura essas linhas em `exportErrors` (e tipos rejeitados em `invalidTypesRejected`) via `Read-MsBuildImportSignals.ps1 -Stage export` → `msbuild.export.signals.json`. **Não** tratar `exitCode=0` sem `exportErrors` vazio como conclusão limpa quando houver erros parciais. Em seletiva com tipos rejeitados, declarar risco de resolução por nome (homonímia).
...
- **Sub-estados de export (operacionais, após inventário e sinais de stdout):**
  ...
  - `exportação concluída, inventário com extras não conciliados` — XPZ existe e inventário obtido, mas em export seletiva há extras, itens pedidos ausentes, `systemModulesPresent` e/ou `systemExternalObjectsPresent` e/ou `attributesTopLevelUnreconciled` num pacote tratado como cirúrgico; declarar ao usuário antes de materializar/importar (somente quando `exportErrors` estiver vazio)
  - `exportação concluída com erros parciais ignorados pela task` — XPZ existe e `exportErrors` não vazio apesar de sucesso aparente da task; reproduzir linhas/amostra no relatório; prioridade sobre sub-estados de inventário “consolidado”
...
9. Se o objetivo for exportação, executar com parâmetros explícitos e conferir o artefato gerado
   ...
   Após exportação com XPZ gerado: ler `packageInventory` e `operationalSubState` no `export.json`; **abrir `package-inventory.json` sempre que** o resumo apontar `attributesTopLevelUnreconciled=true`, `extrasCount > 0`, `totalAttributes > 0` em export seletiva, ou `systemModulesPresent` / `systemExternalObjectsPresent` não vazios (caminho em `nominalInventoryAt`, `packageInventoryPath` ou `artifacts.PackageInventoryPath`); a **lista nominal completa é obrigatória no relatório ao usuário** nesses casos, não opcional — conforme a secção **Inventário do pacote após export seletivo**; só então declarar conclusão limpa ou seguir para materialização/import
...
12. Classificar o resultado como:
   ...
   - em exportação com XPZ gerado, declarar também o sub-estado de export quando `operationalSubState` estiver presente no diagnóstico: `exportação concluída e inventário consolidado`, `exportação concluída, inventário com extras não conciliados`, `exportação concluída com erros parciais ignorados pela task` ou `exportação concluída sem inventário (degradado)` — reproduzir `exportErrors`/`invalidTypesRejected`/`knownStdOutNoise` (top-level) quando existirem; nunca fechar export seletiva relatando só a contagem de `-ObjectList`
...
- [ ] Após export com XPZ gerado, `exportErrors`, `invalidTypesRejected` e `knownStdOutNoise` foram lidos no **top-level** do `export.json` (não só dentro de `stdoutSignals`); ruído `cssproperties-access-denied` ou erros parciais `error :` foram declarados ao usuário quando presentes
- [ ] Quando o `export.json` apontou `extrasCount > 0`, `attributesTopLevelUnreconciled=true`, `totalAttributes > 0` em export seletiva, ou `systemModulesPresent` / `systemExternalObjectsPresent` não vazios, `package-inventory.json` foi lido e a **lista nominal completa** (não apenas `extrasSample` ou contagens) foi reproduzida ao usuário no relatório de fechamento — separada por bloco `<Objects>` e `<Attributes>`
- [ ] Em qualquer pergunta de seguimento sobre conteúdo do pacote na mesma rodada, `package-inventory.json` foi relido antes de responder, em vez de responder pela memória do turno anterior
...
- NEVER declarar export seletiva como conclusão limpa sem `packageInventory` (ou inventário manual) e sem reproduzir no texto ao usuário os totais reais, `systemModulesPresent`, `systemExternalObjectsPresent` e sinais de atributos top-level quando existirem
- NEVER responder perguntas de seguimento sobre conteúdo nominal do XPZ na mesma rodada usando só memória do turno anterior, `extrasSample` ou contagens do `export.json` — reler `package-inventory.json` (ou reexecutar `Get-GeneXusImportPackageObjectInventory.ps1` sobre o `.xpz`) antes de responder
...
- NEVER incluir em pacote tratado como **delta cirúrgico** objetos de módulo de sistema, ExternalObject de plataforma GeneXus (catálogo `gx-system-external-objects.txt`) ou arrasto massivo de atributos top-level sem `Transaction` na lista (`attributesTopLevelUnreconciled`) salvo pedido explícito do utilizador
