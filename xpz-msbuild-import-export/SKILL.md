...
- extras por `DependencyType`/`ReferenceType`, `systemModulesPresent`, `systemExternalObjectsPresent`, atributos top-level em export cirúrgica (`attributesTopLevelUnreconciled`)
...
- linhas `error :` no stdout/stderr (`exportErrors`, `importErrors`, `previewErrors`, `buildErrors` no top-level do JSON, conforme o wrapper)
