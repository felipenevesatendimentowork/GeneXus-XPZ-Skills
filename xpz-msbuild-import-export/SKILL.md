...
- [ ] Quando o `export.json` apontou `extrasCount > 0`, `attributesTopLevelUnreconciled=true`, ou `systemModulesPresent` / `systemExternalObjectsPresent` não vazios, `package-inventory.json` foi lido e a **lista nominal completa** (não apenas `extrasSample` ou contagens) foi reproduzida ao usuário no relatório de fechamento — extras e módulos/ExternalObjects por nome; atributos top-level **por nome** somente quando `attributesTopLevelUnreconciled=true`
