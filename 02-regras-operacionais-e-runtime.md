...
- `Regra operacional`: `CSharpModel\web` e compartilhado entre environments; timestamps frescos ali **nao** provam que `<KbNative>\<EnvDeploy>\web\bin` foi atualizado. Nao usar so `Test-GeneXusRuntimeFreshness.ps1` para concluir validacao deploy em KB multi-environment. Validacao deploy pos-import: wrappers com `-PostImportDeployValidation` checam publicacao em `web\bin` (max timestamp de DLL de objeto excluindo runtime GeneXus/System/Microsoft, ou `*.config`); em Core, `GxNetCoreStartup.dll` e so complementar (warning se velho em build incremental); status `compilou-mas-dll-destino-desatualizada` ou exit **49** quando gate ativo sem evidencia de publicacao.
