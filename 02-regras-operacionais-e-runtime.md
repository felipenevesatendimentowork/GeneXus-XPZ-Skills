...
- `Regra operacional`: objetivo B **nao** exige orquestrador headless nem loop automatico por `kb_environment_names`. Pratica usual e suficiente: apos A bem-sucedido no environment de deploy, **Build na IDE** nos demais environments ativos que a frente ainda cobre (consultar `kb_environment_names` no metadata); conferir compilacao e, se houver duvida, o `.cs` em `<KbNative>\<EnvName>\web\`. Headless repetido com `-EnvironmentName` e opcional, nao padrao.
