...
- `Regra operacional`: `CSharpModel\web` e compartilhado entre environments; timestamps frescos ali **nao** provam que `<KbNative>\<EnvDeploy>\web\bin` foi atualizado. Nao usar so `Test-GeneXusRuntimeFreshness.ps1` para concluir validacao deploy em KB multi-environment. Validacao deploy pos-import: wrappers com `-PostImportDeployValidation` checam `web\bin` conforme `deployment_hosting_kind` (`dotnet-core-self-host` → `GxNetCoreStartup.dll`; `dotnet-framework-iis` → agregado de DLLs); status `compilou-mas-dll-destino-desatualizada` ou exit **49** quando gate ativo.
