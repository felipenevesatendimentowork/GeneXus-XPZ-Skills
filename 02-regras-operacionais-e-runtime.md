...
- `Regra operacional`: `CSharpModel\web` e compartilhado entre environments; timestamps frescos ali **nao** provam que `<KbNative>\<EnvDeploy>\web\bin` foi atualizado. Nao usar so `Test-GeneXusRuntimeFreshness.ps1` para concluir validacao deploy em KB multi-environment.
