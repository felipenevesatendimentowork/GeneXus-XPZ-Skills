...
- `Regra operacional`: a montagem do `import_file.xml` a partir de XMLs de objeto e de um template clonavel deve ser feita por `scripts\Build-GeneXusImportFileEnvelope.ps1`, que executa o gate de colisao (`Test-XpzPackageCollision.ps1`) antes de qualquer escrita — colisao de `_nn` aborta o script sem materializar nenhum byte — clona `KMW`/`Source`/`Dependencies`/`ObjectsIdentityMapping` do template, embute cada objeto via `ImportNode` (impossibilitando declaracao XML interna em `<Objects>`) e executa o gate de envelope automaticamente; concatenacao textual manual e fallback desaconselhado, e nesse caminho o gate de colisao deve ser chamado explicitamente antes da gravacao.
