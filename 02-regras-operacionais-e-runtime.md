...
- `Evidência direta`: motores `Sync-GeneXusXpzToXml.ps1`, inventario e `Test-XpzObjetosDaKbNaming.ps1` aceitam `-ParallelKbRoot` / `-CatalogOverridePath` e usam o mesmo catalogo efetivo (base + override); sync e inventario agregam amostras (`name`, `parent`, `parentType`, snippet XML) em falha ou JSON de triagem.
