...
- `Regra operacional`: `kb-source-metadata.md` deve ser tratado por autoridade de campo: identidade estavel da KB pertence ao setup/resolvedor da KB nativa local, `KMW` vem de XPZ real ou template real comparavel, metadados de materializacao pertencem ao `xpz-sync`, e `last_setup_audit_run_at` pertence ao setup/auditoria.
- `Regra operacional`: quando o sync XPZ/XML atualizar `kb-source-metadata.md` com `-KbMetadataPath`, usar `scripts\Sync-GeneXusXpzToXml.ps1` via `scripts\XpzKbSourceMetadataEditSupport.ps1` para atualizacao **cirurgica** dos campos de materializacao (`updated`, `last_xpz_materialization_run_at`, `source_xpz`, `source_refresh_status` e tabelas `KMW`/`Source`/`Source/Version`), preservando `last_setup_audit_run_at`, demais frontmatter fora do escopo e EOL dominante via `scripts\XpzTextFileEolSupport.ps1`; nao regerar o arquivo inteiro nesse caminho.
- `Regra operacional`: apos auditoria de setup bem-sucedida, preferir `scripts/Set-XpzSetupAuditTimestamp.ps1` (wrapper local `Set-*KbSetupAuditTimestamp.ps1`) para gravar `last_setup_audit_run_at`; oferecer na mesma sessao quando `Test-*KbSetupFreshness.ps1` tiver exigido auditoria completa por campo ausente ou defasado e o gate tiver passado — adiar so com recusa ou adiamento explicito do usuario.
...
- `Regra operacional`: `metadata/deploy` diferente de `OK` em `Test-*KbSetupAudit.ps1` (`PENDENTE` ou `BLOCK`) indica campos de environment/deploy ausentes ou `kb_environment_names` inconsistente ou legado (ex.: nomes tipicos de scan por pastas `web\`). Complementa `metadata wrapper: OK`; impede declarar `materializado_e_indice_validado` ate perguntar ao usuario os nomes exatos, gravar via `Set-*KbSourceMetadataDeployment.ps1` com `-KbEnvironmentNames` e validar cada nome via MSBuild (`SetActiveEnvironment` headless). Scan/inventario automatico por pastas (`-InventoryFromKbNativePath`, `-InventoryFromGeneXusMsBuild`) foi removido. `Test-XpzWrapperInventory.ps1` sinaliza wrapper local que ainda usa descoberta automatica ou omite `-KbEnvironmentNames`.
