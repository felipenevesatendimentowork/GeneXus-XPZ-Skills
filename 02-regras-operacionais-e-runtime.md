...
- `Regra operacional`: wrappers MSBuild que suportam watcher devem expor o mesmo contrato (`-StartWatcher`, `-MonitorLogPath`, `-WatcherIntervalSeconds`, `-WatcherSilenceThresholdSeconds`) e registrar `watcherContext`/`timing.phases` por meio de `scripts\GeneXusMsBuildWatcherSupport.ps1`; `-StartWatcher` sem `-MonitorLogPath` deve bloquear cedo com exit 46. Em pacote amplo ou com muitos `WorkWithForWeb`, importacao real sem watcher deve ser tratada como excecao operacional justificada, porque a importacao pode disparar regeneracao dos objetos derivados do pattern e durar tempo significativo sem feedback visual.
