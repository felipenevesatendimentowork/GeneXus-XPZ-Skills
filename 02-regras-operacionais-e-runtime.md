...
- `Regra operacional`: em busca textual pontual de `Source` de `WebPanel`, usar `scripts\Search-GeneXusXmlSourceBlock.ps1` como ponto de entrada publico para separar `events`, `layout`, `code` e `serialized`; match bruto em `GxMultiForm` inline nao prova comportamento de code-behind
...
- `Regra operacional`: `scripts-maintenance/` contem ferramentas de manutencao desta base e campanhas empiricas; esses scripts nao sao runtime publico das skills em pastas paralelas de KB, mas continuam sob contrato de parse `pwsh` 7.4+.
- `Regra operacional`: pontos de entrada publicos em `scripts/*.ps1` devem declarar `#requires -Version 7.4` para falhar cedo em host PowerShell antigo; excecao nominal: `Test-XpzPowerShellRuntime.ps1` deve continuar executavel em Windows PowerShell 5.1 para localizar `pwsh` e emitir bloqueio claro quando o runtime suportado estiver ausente.
- `Regra operacional`: a validacao automatica de parse PowerShell da base e `scripts/Test-PsScriptsParse.ps1`, tambem executada por `.github/workflows/parse-ps-scripts.yml`; ela deve cobrir `scripts/*.ps1`, `scripts-maintenance/*.ps1` e `.example.ps1` das skills fora de `historico/` sob o contrato `pwsh` 7.4+.
- `Regra operacional`: `xpz-kb-parallel-setup` deve exigir wrapper local `Test-*KbPowerShellRuntime.ps1` como primeiro gate de uso operacional da pasta paralela; se `pwsh` 7.4 LTS ou superior estiver ausente, o wrapper deve retornar bloqueio e a pasta nao deve prosseguir para sync, indice, auditoria substantiva ou empacotamento.
...
- `Regra operacional`: ao comparar itens esperados de `IncludeItems` com `importedItems`, o diagnostico compacto deve preservar `expectedItemsRaw`/`importedItemsRaw`, emitir `expectedItemsCanonical`/`importedItemsCanonical` e registrar `itemAliasMatches`; `Panel:Nome` e `SDPanel:Nome` sao equivalentes para esse matching, sem esconder a forma crua emitida pelo GeneXus.
- `Regra operacional`: quando o diagnostico JSON de preview/import trouxer `compactSignals`, essa deve ser a leitura compacta preferencial dos sinais de importacao; o arquivo lateral `msbuild.import.signals.json` e fallback auditavel quando `compactSignals` estiver ausente, nulo ou insuficiente.
- `Regra operacional`: quando a task `Genexus.MsBuild.Tasks.Import` nao expuser `UpdateFile` nem `ImportKBInformation`, o wrapper deve bloquear cedo esses parametros quando explicitamente pedidos em valor nao neutro (`UpdateFile` com caminho informado, `ImportKbInformation=true`); valor neutro (`ImportKbInformation=false` ou `UpdateFile` omitido) deve ser tratado como nao emissao, sem bloqueio. O bloqueio nao deve ser contornado por repeticao silenciosa sem o parametro: a divergencia entre chamada/wrapper e assinatura efetiva deve ser declarada nominalmente antes de qualquer correcao.
...
- `Regra operacional`: wrappers compartilhados e wrappers locais de empacotamento XPZ devem emitir JSON de maquina por padrao no stdout e nao usar `-AsJson`; wrappers locais antigos devem ser atualizados pela skill `xpz-kb-parallel-setup`. Bloqueios esperados devem retornar objeto estruturado com `status`, `exitCode`, `stage`/`reason`, `blockingReasons` e campos acionaveis (por exemplo `nextFreeNN` em colisao de pacote), sem ANSI nem stack trace no canal de maquina. Para conceitos equivalentes, preferir `-InputPath` para caminho de entrada e `-ObjectList` para lista nominal `Tipo:Nome`; `-Path` e `-ObjectNames`, quando aceitos, sao aliases/compatibilidade.
- `Regra operacional`: os wrappers MSBuild que suportam watcher (`Invoke-GeneXusKbBuildAll.ps1`, `Invoke-GeneXusKbSpecifyGenerate.ps1`, `Test-GeneXusXpzImportPreview.ps1`, `Invoke-GeneXusXpzExport.ps1` e `Invoke-GeneXusXpzImport.ps1`) devem expor o mesmo contrato (`-StartWatcher`, `-MonitorLogPath`, `-WatcherIntervalSeconds`, `-WatcherSilenceThresholdSeconds`) e registrar `watcherContext`/`timing.phases` por meio de `scripts\GeneXusMsBuildWatcherSupport.ps1`; `-StartWatcher` sem `-MonitorLogPath` deve bloquear cedo com exit 46. Em `Invoke-GeneXusKbBuildAll.ps1` e `Invoke-GeneXusKbSpecifyGenerate.ps1`, watcher e fluxo padrao; em preview/export/import, e padrao para execucao longa. **Fora** da **Decisao pos-gates**, em importacao real de pacote amplo ou com muitos `WorkWithForWeb`, executar sem watcher deve ser tratado como excecao operacional justificada, porque a importacao pode disparar regeneracao dos objetos derivados do pattern e durar tempo significativo sem feedback visual; essa excecao por justificativa **nao** se aplica quando valer **Decisao pos-gates** (regra dedicada abaixo).
- `Regra operacional`: privilegiar `PreviewMode` antes de importacao real apenas quando import real ainda nao foi autorizado na sessao ou a rodada for exploratoria; contrato detalhado em `xpz-msbuild-import-export/SKILL.md` (secao **Decisao pos-gates**).
- `Regra operacional`: quando o usuario ja autorizou importacao real headless na mesma sessao, `Test-GeneXusImportFileEnvelope.ps1` retornou `apto para prosseguir` e o inventario do pacote (passo 6c de `xpz-msbuild-import-export`) passou sem bloqueio de extras, o agente deve executar `Invoke-GeneXusXpzImport.ps1` na mesma rodada com `-StartWatcher` e `-MonitorLogPath` (obrigatorios nesse caminho; sem excecao por justificativa de ausencia de watcher); nao encerrar so pelo envelope apto nem exigir `Test-GeneXusXpzImportPreview.ps1` obrigatorio antes do import nesse cenario.
- `Regra operacional`: autorizacao de import real na sessao nao cobre objetos extras, modulos/ExternalObjects de plataforma nao pedidos nem `attributesTopLevelUnreconciled` em pacote cirurgico; nesses casos **ABORT** antes do import e listar o pacote completo ao usuario — anti-padroes **parada apos envelope apto** e **reempacotar lixo de export** em `xpz-msbuild-import-export/SKILL.md`.
- `Regra operacional`: em trilha MSBuild headless, distinguir **Categoria A** (comportamento esperado da task — extras de inventario, modulos de plataforma, atributos top-level em export cirurgica; sinalizar ao usuario sem rebaixar `exitCode` quando so A estiver presente) de **Categoria B** (rejeicao no log — linhas `error :`, `invalidTypesRejected`, `exportErrors`/`importErrors`/`previewErrors`/`buildErrors`/`specifyErrors`). Quando `executionEvidence.msBuildExitCode=0` mas Categoria B estiver populada, os wrappers rebaixam o `exitCode` classificado para **48** (`scripts/GeneXusMsBuildCategoryBSupport.ps1`, catalogo em `scripts/msbuild-exit-codes.catalog.json`); artefato no disco nao autoriza entrega operacional limpa. Na **Decisao pos-gates**, `exitCode=48` bloqueia import real — autorizacao da sessao nao cobre B.
- `Regra operacional`: distinguir **fallback silencioso por nome** (pedido `Tipo:Nome` sem identidade demonstrada no inventario, com `exitCode=0` e sem Categoria B — homonimia, resolucao errada da task GeneXus) de **divergencia de rotulo Export** (`exportTaskLabel` / `10a-gx-export-task-labels.md`, ex. lista `WorkWith:Nome`, pacote `WorkWithForWeb:Nome` — motor em `Get-GeneXusImportPackageObjectInventory.ps1` preenche `deltaComparison.aliasResolutions[]` com regra `exportTaskLabel`). Categoria B e tipo invalido no log nao sao silenciosos. Com `exitCode=0` e identidade nao demonstrada (sem match literal nem `aliasResolutions[]`), **ABORT** antes de materializar/importar/sync — anti-padrao e CONSTRAINT em `xpz-msbuild-import-export/SKILL.md`. Export seletivo via `Invoke-GeneXusXpzExport.ps1` e import/preview seletivo com `-IncludeItems` via `Invoke-GeneXusXpzImport.ps1` / `Test-GeneXusXpzImportPreview.ps1` exigem `-ParallelKbRoot` ou `-IndexPath`; homonimia ou indice invalido/ausente bloqueia com **exit 35** (`objectListPreflight`, estagios `pre-export-identity` / `pre-import-identity`); ausencia no indice sem homonimia gera aviso e segue MSBuild (objeto pode existir so na KB nativa). Inventario pos-pacote continua obrigatorio no import.
- `Regra operacional`: indice numerico de `exitCode` dos wrappers MSBuild GeneXus XPZ — fonte canônica `scripts/msbuild-exit-codes.catalog.json` (verificacao mecanica `scripts/Test-MsBuildExitCodesCatalog.ps1`). Para **exit 46**, nunca inferir causa apenas pelo numero: consultar `causes[]`, `summary` e `blockingReasons` no JSON do wrapper (watcher sem `-MonitorLogPath`, wide rebuild/reorg, combinacoes invalidas de switches — ver catálogo). Para demais exits, distinguir `exitCode` classificado de `executionEvidence.msBuildExitCode` conforme `legend` do catálogo e `10-base-operacional-msbuild-headless.md`.
- `Regra operacional`: quando `exitCode=46` representar bloqueio preventivo de `MSBuild` concorrente na mesma KB, a rodada deve parar e reportar o conflito; a trilha compartilhada nao enfileira, nao aguarda automaticamente e nao faz retentativa em loop.
- `Regra operacional`: em trilha `MSBuild`, distinguir sempre `sucesso operacional da chamada`, `preview apenas`, `sucesso operacional com falha no pos-processamento` (MSBuild concluiu com `exitCode=0` — valor bruto canônico em `executionEvidence.msBuildExitCode=0` — e a evidencia primaria do log bruto esta presente — em import/export: `__IMPORTED_ITEM__` ou `__EXPORTED_FILE__` mais arquivo XPZ existente; em build-all: `__BUILDALL_DONE__=true` e/ou `observedContext.BuildAllDone=true` no diagnostico parcial; em specify-generate: `__SPECIFY_DONE__=true` e/ou `__GENERATE_DONE__=true` e/ou `observedContext.SpecifyDone`/`observedContext.GenerateDone` no diagnostico parcial —, mas o pos-processamento local do wrapper falhou e o JSON saiu com `postProcessingFailed=true`), `preview apenas com falha no pos-processamento` (analogo ao anterior na fase de preview: MSBuild concluiu com `exitCode=0` — valor bruto canônico em `executionEvidence.msBuildExitCode=0` — sem alterar a KB e a evidencia primaria do log bruto esta presente, mas o pos-processamento local do wrapper falhou e o JSON saiu com `postProcessingFailed=true`) e `confirmacao funcional posterior na IDE`. O campo `exitCode` no JSON dos wrappers e o valor classificado pelo wrapper (0/32/41/42/48/...); o valor bruto da task MSBuild fica preservado canonicamente em `executionEvidence.msBuildExitCode`; `msBuildExitCode` top-level, quando existir, e compatibilidade transitoria e deve duplicar o valor canônico, enquanto `blockingReasons` deve concentrar causas acionaveis.
- `Regra operacional`: `diagnosticDegraded=true` em diagnostico MSBuild indica diagnostico parcial depois da execucao, nao falha operacional automatica. Ele pode coexistir com `postProcessingFailed=false` quando a saida principal foi montada, mas algum complemento (como `msbuild.import.signals.json`) falhou ou ficou parcial. A classificacao final deve considerar `executionEvidence`, marcas do log bruto e a skill responsavel pelo wrapper.
- `Regra operacional`: erro de leitura de `GxImport.log` por arquivo bloqueado deve ser classificado como diagnostico degradado (`gxImportLogReadStatus=locked`, `diagnosticDegraded=true`) quando houver evidencias primarias de stdout/stderr suficientes para classificar a chamada; esse sintoma nao deve virar causa principal de falha operacional por si so.
...
- `Evidencia direta`: na KB `FabricaBrasil18`, exportacao headless de `WebPanel:wpProcessaArquivoDeTransacaoDePagamento` com `ObjectList` preenchido e `DependencyType`/`ReferenceType` omitidos gerou pacote com 25 objetos e 50 atributos top-level; a mesma chamada com `DependencyType="None"` e `ReferenceType="None"` gerou pacote com exatamente 1 objeto e 0 atributos top-level.
- `Regra operacional`: em exportacao seletiva/cirurgica cujo objetivo seja obter somente os objetos listados em `Objects`/`ObjectList`, passar `DependencyType="None"` e `ReferenceType="None"` como prevencao na origem. Isso nao substitui inventario pos-export: o pacote gerado continua devendo ser confrontado com o delta declarado antes de materializar, importar ou sincronizar. Valores aceitos alem de `None` e o default formal quando esses atributos sao omitidos permanecem a confirmar.
...
- `Regra operacional`: em stdout de execucao MSBuild, a janela iniciada por `Executando eventos pos-construcao ...` e encerrada pelo proximo separador `==========` indica eventos pos-build configurados na KB que dispararam processos externos; registrar como warning separado, pois esses processos podem incluir deploys automaticos ou outras acoes com efeito colateral fora do escopo da verificacao. Para logs sem esse marcador, os wrappers mantem fallback historico para linhas `start c:` ou `start cmd`.
...
- `Regra operacional`: `BuildAll` via MSBuild com `ForceRebuild=true` equivale a `Rebuild All` da IDE — regenera TODOS os objetos da KB independentemente de mudanca, nao apenas os alterados desde o ultimo build. Em KB grande pode durar horas e regenerar centenas/milhares de objetos. Os wrappers `Invoke-GeneXusKbBuildAll.ps1` e `Invoke-GeneXusKbSpecifyGenerate.ps1` bloqueiam `ForceRebuild=true` por padrao (exit 46); so pode ser habilitado via `-AllowWideRebuild` com confirmacao explicita do usuario pela frase exata `entendo que isto pode regerar a KB inteira e aceito o custo`.
- `Regra operacional`: `CompileMains=true` em `BuildAll` compila tambem os objetos Main alem do Developer Menu; `DetailedNavigation=true` em `BuildAll` ou `SpecifyGenerate` executa navegacao detalhada durante a especificacao. Essas opcoes podem ampliar muito o custo de uma validacao cotidiana em KB grande. Os wrappers `Invoke-GeneXusKbBuildAll.ps1` e `Invoke-GeneXusKbSpecifyGenerate.ps1` bloqueiam essas opcoes por padrao (exit 46); so podem ser habilitadas via `-AllowCostlyBuildOptions` com confirmacao explicita do usuario pela frase exata `entendo que estas opcoes podem ampliar muito o custo do build e aceito executar`.
- `Regra operacional`: o gate de regeneracao ampla (`-AllowWideRebuild`), o gate de opcoes caras de build (`-AllowCostlyBuildOptions`) e o gate de reorg (`-AllowReorg`) sao independentes: autorizar um nao autoriza os outros. Cada operacao ampla ou cara exige sua propria confirmacao por frase exata especifica.
...
- `Regra operacional`: `kb-source-metadata.md` deve ser tratado por autoridade de campo: identidade estavel da KB pertence ao setup/resolvedor da KB nativa local, `KMW` vem de XPZ real ou template real comparavel, metadados de materializacao pertencem ao `xpz-sync`, e `last_setup_audit_run_at`/`setup_contract_signature_*` pertencem ao setup/auditoria.
- `Regra operacional`: quando o sync XPZ/XML atualizar `kb-source-metadata.md` com `-KbMetadataPath`, usar `scripts\Sync-GeneXusXpzToXml.ps1` via `scripts\XpzKbSourceMetadataEditSupport.ps1` para atualizacao **cirurgica** dos campos de materializacao (`updated`, `last_xpz_materialization_run_at`, `source_xpz`, `source_refresh_status` e tabelas `KMW`/`Source`/`Source/Version`), preservando `last_setup_audit_run_at`, `setup_contract_signature_*`, demais frontmatter fora do escopo e EOL dominante via `scripts\XpzTextFileEolSupport.ps1`; nao regerar o arquivo inteiro nesse caminho.
- `Regra operacional`: apos auditoria de setup bem-sucedida, preferir `scripts/Set-XpzSetupAuditTimestamp.ps1` (wrapper local `Set-*KbSetupAuditTimestamp.ps1`) para gravar `last_setup_audit_run_at` e a assinatura atual do contrato de setup (`setup_contract_signature_version`/`setup_contract_signature_hash`); oferecer na mesma sessao quando `Test-*KbSetupFreshness.ps1` tiver exigido auditoria completa por campo ausente, assinatura ausente ou assinatura defasada e o gate tiver passado — adiar so com recusa ou adiamento explicito do usuario.
- `Regra operacional`: ao concluir auditoria minima de pasta paralela (`xpz-kb-parallel-setup`), o agente deve montar um **plano consolidado de correcoes** com tudo que a skill classificar como corrigivel na pasta, oferecer execucao na mesma sessao apos aprovacao quando exigida, e so tratar adiamento como legitimo com recusa ou adiamento explicito do usuario — nao fechar apenas com diagnostico quando houver item corrigivel.
- `Regra operacional`: reconciliacao aprovada de identidade estavel deve usar `scripts\Resolve-GeneXusKbIdentity.ps1` para resolver a KB nativa local e `scripts\Update-XpzKbSourceMetadataIdentity.ps1` para escrita localizada em `kb-source-metadata.md`; o atualizador preenche campos ausentes e bloqueia divergencias nao vazias sem aprovacao explicita de sobrescrita.
- `Regra operacional`: `metadata wrapper` diferente de `OK` por identidade estavel ausente, parse incompleto ou falha funcional nao e ressalva informativa; impede declarar `materializado_e_indice_validado` ate que o metadado seja reconciliado ou o wrapper seja corrigido. `GATE_OK` e indice valido provam apenas a aptidao do indice, nao a integridade do contrato de metadata.
- `Regra operacional`: `metadata/deploy` diferente de `OK` em `Test-*KbSetupAudit.ps1` (`PENDENTE` ou `BLOCK`) indica campos de environment/deploy/output ausentes, `kb_environment_names` inconsistente ou legado (ex.: nomes tipicos de scan por pastas `web\`), ou mapeamento `kb_environment_output_dirs`/`kb_environment_web_dirs` ausente/divergente. Complementa `metadata wrapper: OK`; impede declarar `materializado_e_indice_validado` ate perguntar ao usuario os nomes exatos, o diretorio de output de cada environment, gravar via `Set-*KbSourceMetadataDeployment.ps1` com `-KbEnvironmentNames` e `-KbEnvironmentOutputDirs`, e validar cada nome via MSBuild (`SetActiveEnvironment` headless). Scan/inventario automatico por pastas (`-InventoryFromKbNativePath`, `-InventoryFromGeneXusMsBuild`) foi removido. `Test-XpzWrapperInventory.ps1` sinaliza wrapper local que ainda usa descoberta automatica ou omite `-KbEnvironmentNames`/`-KbEnvironmentOutputDirs`.
- `Regra operacional`: `Source` ausente ou incompleto pode ser metadata incompleto da propria KB; `Source/@kb` preenchido com GUID diferente da KB nativa local é pacote de outra KB. Agente nao deve trocar esse GUID para normalizar o pacote nem prosseguir com import headless; nesse caso, orientar importacao manual pela IDE.
...
- `Regra operacional`: o indice esta apto para triagem ampla apenas quando `last_index_build_run_at` for igual ou posterior a `last_xpz_materialization_run_at` lido nominalmente em `kb-source-metadata.md`, `inventory_validation_status` estiver literalmente `OK` no `index-metadata` e `extractor_signature_version`/`extractor_signature_hash` na metadata do SQLite coincidirem com o motor em `scripts/Build-KbIntelligenceIndex.py` do repositorio ativo (comparacao via `scripts/GeneXusKbIntelligenceExtractorContract.ps1` ou gate `Test-*KbIndexGate.ps1`).
- `Regra operacional`: antes de `who-uses`, `what-uses`, `impact-basic` ou `functional-trace-basic`, conferir no catalogo efetivo (`scripts/gx-object-type-catalog.json` + `scripts/gx-object-type-catalog.override.json` na pasta paralela quando existir) se o tipo tem `queryableByKbIntelligence=true`; quando for `false`, `Query-KbIntelligenceIndex.py` devolve exit `11`, `blocked=true` e `reason=QUERY_NOT_SEMANTIC_FOR_TYPE` — **nao** tratar como zero dependencias; preferir `object-info`, `search-objects`, `list-by-type` ou XML pontual (lista canônica no JSON).
- `Regra operacional`: o indice materializa chamadas resolviveis em `Property Formula` de `Attribute` para `Procedure`, `WebPanel` e `DataProvider` (mesmos padroes de `Source` efetivo); `who-uses` em procedure deixa de ignorar esse uso, mas nao substitui leitura da formula quando a semantica da expressao for decisiva.
- `Regra operacional`: em `ExternalObject`, o indice diferencia referencia declarativa por `ATTCUSTOMTYPE` de uso efetivo por chamada de metodo em variavel `exo:<ExternalObject>` no `Source` efetivo; a chamada efetiva gera relacao `calls_external_object_method` com regra `source_external_object_method`, mas ainda nao substitui leitura do XML quando a pergunta depender de assinatura, parametros ou binding nativo.
- `Regra operacional`: gravabilidade por `Transaction`/`Level`/`Attribute` fica materializada em `transaction_attribute_writability` no build (`schema_version=3`, extrator `6`); `transaction-writable-attributes` no indice tem paridade com `Test-GeneXusTransactionWritability.ps1` quando `Test-GeneXusKbIntelligenceWritabilityParity.ps1` passa na pasta paralela. Fonte canonica do algoritmo: `scripts/GeneXusTransactionWritabilityCore.py`; os gates PowerShell `Test-GeneXusTransactionWritability.ps1` e `Test-GeneXusNewWritableTargets.ps1` sao fachadas via `GeneXusTransactionWritabilitySupport.ps1` (sem reimplementacao duplicada). Classificacao `unclassified-*` (`writable=null`) continua bloqueando `New`/atribuicao sem leitura adicional do XML. Blocos `New` em `Procedure` ainda exigem `Test-GeneXusNewWritableTargets.ps1`.
- `Regra operacional`: classes CSS ficam catalogadas em `css_class` no build (`schema_version=3`, extrator `6`), com dimensoes `model` (`legacy-theme` de `ThemeClass` | `design-system` de SCSS no Part Styles `c6b14574-...` de `DesignSystem`) e `origin` (`kb-authored` | `packaged-module`); o uso vira relacao `uses_css_class` (resolvivel, `evidence_role` `css_layout`/`css_event`) ou `uses_css_class_dynamic` (atribuicao `.Class=` por variavel/`Format()`, nao atribuivel a uma classe). Consultas `css-classes` (catalogo; visao padrao so `kb-authored`, lookup nominal `case-sensitive` nunca filtra origem; `legacy-theme` marcado depreciado) e `css-class-usage` (uso; declara `dynamic_uses_total` e `found_in_catalog`, e lista usadas-mas-nao-catalogadas). Localizador = objeto + snippet (no layout tudo numa linha so, `line` pouco util). E acelerador de triagem com frescor por gate — renomear/remover classe exige conferencia por busca literal no XML. Ver `08` e `scripts/README-kb-intelligence.md`.
- `Regra operacional`: `AGENTS.md` e `README.md` locais da pasta paralela da KB nao devem gravar timestamps literais de materializacao ou indice; devem apontar para `kb-source-metadata.md`, para `-Query index-metadata` do wrapper local e para o gate efetivo. Timestamp literal nesses markdowns e drift documental e pendencia operacional real; a correcao e substituir por ponteiros para as fontes autoritativas, nao atualizar o valor duplicado.
...
- `Regra operacional`: para apenas re-carimbar (bump) o `lastUpdate` de um XML da frente JA editado (rodada 2+), sem copiar do acervo nem aplicar delta, usar `scripts/Set-GeneXusXmlLastUpdate.ps1` (`-InputPath`, `-BaselineXmlPath` opcional, `-DryRun`, `-AsJson`); recalcula `max(UtcNow + margem, baseline + margem)` e grava in-place com backup e validacao de well-formedness, em vez de busca-e-troca manual do atributo. Sem `-BaselineXmlPath`, o baseline e o proprio arquivo — suficiente para ficar acima do objeto vivo na KB em rodadas subsequentes, ja que o GeneXus preserva o `lastUpdate` importado como Modified Date; para garantir acima do acervo, apontar `-BaselineXmlPath` para o XML oficial em `ObjetosDaKbEmXml`.
...
- `Regra operacional` (anti-padrão "editar acervo esperando que o pacote pegue"): editar `ObjetosDaKbEmXml` com a intencao de que o proximo empacotamento use essa versao e um anti-padrao. O motor de empacotamento le os XMLs da subpasta ativa da frente (`ObjetosGeradosParaImportacaoNaKbNoGenexus/<Frente>/`), nunca do acervo. A edicao do acervo nao afeta o conteudo do pacote; a unica forma de influenciar o empacotamento e editar o XML na pasta da frente. O gate 9-FD (`Test-GeneXusFrontAcervoDrift.ps1`, sempre executado pelo wrapper `New-XpzImportPackage.ps1` em modo fail-closed: `-AcervoPath` opcional, acervo canonico `<RepoRoot>/ObjetosDaKbEmXml` auto-resolvido quando omitido, empacotamento bloqueado sem acervo resolvivel) detecta drift de `lastUpdate` entre frente e acervo, mas o anti-padrao conceitual permanece mesmo quando o `lastUpdate` coincide: o acervo e fonte normativa, nao area de rascunho. Quem precisa alterar um objeto para importacao deve copiar do acervo para a frente, editar a copia e, se necessario, bump o `lastUpdate`; nunca editar o acervo diretamente. Para copiar e bump em um passo, use `Copy-GeneXusAcervoToFront.ps1` (`-FrontFolder <pasta-da-frente> -AcervoFolder <ObjetosDaKbEmXml>`). Para seed inicial de objeto ainda ausente na frente, informar alvo explicito por `-ObjectList`, `-ObjectNames` ou `-ObjectGuids`; frente vazia sem alvo explicito continua `not-applicable`.
...
- `Regra operacional`: em Panel SD, `detail/@events` e fonte de comportamento serializado; acoes em layout com `onClickEvent` devem ser confrontadas com os eventos persistidos nesse atributo antes de concluir cobertura ou equivalencia entre Panels.
- `Regra operacional`: quando um Panel SD comparavel usar `<action onClickEvent="'Nome'">` com `Event 'Nome'`, preservar esse vinculo; nao sintetizar `Event Controle.Tap` sem evidencia equivalente em Panel real comparavel da mesma KB.
- `Regra operacional`: o aviso de acoplamento `level id`/`layout id` so pode deixar de ser ressalva quando o par do pacote for localizado em template ou referencia comparavel explicitamente informada; mera presenca de dois GUIDs nao comprova coerencia.
- `Regra operacional`: ao validar envelope com `scripts\Test-GeneXusImportFileEnvelope.ps1`, passar `-PanelReferencePath <objeto-ou-pacote-comparavel>` quando houver referencia real disponivel; em Panel, ausencia dessa referencia retorna `panel-level-layout-unverified` em `warnings`, referencia sem o mesmo par retorna `panel-level-layout-suspicious` em `warnings`, e par comprovado retorna `panel-level-layout-confirmed` somente em `information`, sem rebaixar o status por essa verificacao.
...
- `Regra operacional`: arquivos `.md` e `.ps1` deste repositorio usam UTF-8 sem BOM; o gate `Test-GeneXusUnexpectedCharacter.ps1` (invocado pela pre-push mecanica) detecta caracteres Unicode fora dos blocos esperados para conteudo em pt-BR/es/en (CJK, Cirilico, Arabic, etc.) em linhas adicionadas ao diff, excluindo `historico/` e blocos de codigo cercados em `.md`; findings sao consultivos (severity `warn`); revisar e descartar falso positivo antes de concluir a pre-push.
- `Regra operacional`: scripts `.ps1` desta raiz que gravem texto em UTF-8 sem BOM devem carregar `scripts/Utf8NoBomEncodingSupport.ps1` e usar `Get-Utf8NoBomEncoding`; manter construtores inline de `UTF8Encoding` apenas quando o objetivo for leitura/deteccao de encoding, BOM deliberado ou validacao estrita de bytes.
...
- `Regra operacional`: em `WebPanel`, um botao e declarado uma unica vez no layout, em uma de duas formas serializadas do mesmo controle Button — `<action controlName= onClickEvent= caption=>` ou `<ucw gxControlType="-2133704903" PATTERN_ELEMENT_CUSTOM_PROPERTIES=...>` (nome e evento dentro das Properties escapadas) — e disparado por um Event de usuario nomeado (`Event 'Nome'`) ou padrao (`Enter`/`Back`); a escolha da forma e detalhe de serializacao, nao de modelagem (padrao observado: `ucw` em menus, `<action>` no restante).
- `Regra operacional`: nao confundir tres usos distintos no XML de `WebPanel` — botao de layout (`<action controlName= onClickEvent=>` ou `<ucw>` Button) dentro de `<layout>`; acao de pattern `WorkWithForWeb` (`<action name=>` dentro de `<actions>`); e referencia de propriedade em codigo de evento (`Botao.Visible`/`.Icon`/`.Enabled`), que manipula controle ja declarado e nao e declaracao nem evento; N ocorrencias do nome de um botao no `Source` nao significam N botoes.
- `Regra operacional`: para inspecionar o shape de `WebPanel` classico sem despejar `CDATA`, usar o bloco `webpanel` de `scripts/Get-GeneXusObjectSummary.ps1` (tabelas com `tableType` Flex/Responsive e `depth`, controles, botoes nas duas formas, eventos e bloco `coverage`); `gxControlType` e resolvido pelo catalogo versionado `scripts/gx-ucw-gxcontroltype-catalog.json` e, quando ausente, reportado em `unknownUcwControlTypes`, nunca omitido — nao ler controle ausente do resumo como prova de ausencia sem checar `coverage`.
- `Regra operacional`: para adicionar um botao a `WebPanel`, preferir `scripts/Add-GeneXusButton.ps1`, que insere `<cell>` antes ou apos a celula de um controle folha em tabela Flex (ancoras `-BeforeControlName`/`-AfterControlName`, mutuamente exclusivas; com stub de Event e bump de `lastUpdate`, por insercao cirurgica); em tabela Responsive com `responsiveSizes` preenchido o helper aborta fail-closed (`RESPONSIVE_UNSAFE`) em vez de reescrever o array de breakpoints — nesse caso, gerar o snippet e ajustar `responsiveSizes` manualmente.
...
- `Regra operacional`: a montagem do `import_file.xml` a partir de XMLs de objeto deve usar motor estruturado compartilhado, nunca concatenacao textual manual. Para montagem direta com template clonavel, usar `scripts\Build-GeneXusImportFileEnvelope.ps1` (com `-AcervoPath <ObjetosDaKbEmXml>` obrigatorio, gate de `lastUpdate` sempre ativo e declaracao de modificados por `-ModifiedObjectNames` ou `-ModifiedObjectGuids`), que executa o gate de colisao (`Test-XpzPackageCollision.ps1`) antes de qualquer escrita — colisao de `_nn` aborta o script sem materializar nenhum byte — clona `KMW`/`Source`/`Dependencies`/`ObjectsIdentityMapping` do template, clona top-level `Attributes` do template quando nao houver atributos explicitos passados ao helper, embute cada objeto via `ImportNode` (impossibilitando declaracao XML interna em `<Objects>`) e executa o gate de envelope automaticamente; para `Panel`, esse gate recebe automaticamente o proprio `-TemplatePackagePath` como `-PanelReferencePath` e propaga `information`/`warnings` conforme a confirmacao do par `level id`/`layout id`. Para montagem por frente da pasta paralela, usar `scripts\New-XpzImportPackage.ps1` como wrapper do motor `scripts\New-XpzImportPackage.py`, que classifica raizes top-level `Object`/`Attribute`, aplica gate de colisao e pode usar `kb-source-metadata.md` para envelope minimo ou `-TemplatePackagePath` para clonar pacote real comparavel, incluindo top-level `Attributes` do template quando a frente nao trouxer raizes `Attribute` explicitas. O wrapper executa sempre, antes do empacotamento, o gate de drift frente-vs-acervo (`Test-GeneXusFrontAcervoDrift.ps1`, passo 9-FD) em modo fail-closed: `-AcervoPath` e opcional e, quando omitido, o acervo canonico `<RepoRoot>/ObjetosDaKbEmXml` e resolvido automaticamente; sem acervo resolvivel o empacotamento e bloqueado, e o JSON reporta `acervoResolvedBy` (`explicit` ou `convention`). O gate bloqueia XMLs com `lastUpdate` mais antigo que o acervo. `-TemplatePackagePath` pode apontar para XML/import_file.xml ou para `.xpz` que contenha XML `ExportFile`; em pacote misto/complexo, preferir template real comparavel. Envelope minimo derivado de metadados e apoio operacional para casos simples, nao formato universal.
- `Regra operacional`: antes de usar `kb-source-metadata.md` ou `-TemplatePackagePath` para montar pacote importavel, confrontar a identidade `Source/@kb` do envelope/template com a KB nativa local esperada quando essa informacao estiver disponivel. Divergencia indica pacote de outra KB e bloqueia import headless por agente; o usuario deve usar a IDE se quiser avaliar esse import.
...
- `Evidência direta`: em `WebPanel` com Tab externa contendo outra Tab interna cujas sub-abas exibem campos bindados em `&Sdt.<membro>` via data attributes, a sub-aba default da Tab interna aparece vazia na primeira ativacao da aba externa correspondente, mesmo com o SDT ja populado server-side; o workaround manual eh clicar em outra sub-aba e voltar.
- `Inferência forte`: o re-bind das data attributes nas sub-abas internas eh disparado client-side quando o framework JS recebe a chamada `ajax_rsp_assign_sdt_attri(..., AV<NomeSdt>, AV<NomeSdt>)` no `.cs` gerado; sem evento server-side que marque o SDT como mutado, essa chamada nao eh emitida.
- `Regra operacional`: para forcar a emissao, declarar `Event <OuterTab>.TabChanged` com branch por `<OuterTab>.ActivePageControlName`, dentro da branch fazer `touch opaco` em UM membro do SDT via proc helper identidade (`parm(in:&T, out:&T)` com corpo `&Volta = &Vinda`) e chamar `<InnerTab>.SelectTab(2)` seguido de `<InnerTab>.SelectTab(1)`; o par `(2);(1)` emula o click manual do usuario e gera duas chamadas `executeUsercontrolMethod "SelectTab"` no `.cs`, suficientes para o cliente processar o re-bind.
...
- `Anti-pattern`: `&sdt = &sdt`, `&sdt.<membro> = &sdt.<membro>`, `&sdt.<campo> = &sdt.<campo> + ""`, `&sdt.FromJson(&sdt.ToJson())`, `&sdt.FromJSonString(&sdt.ToJSonString(false))` e `Refresh` sozinho nao funcionam — os primeiros sao eliminados como no-op pelo dead-code do gerador (handler stripado do `.cs` sem erro nem warning), `FromJson/ToJson` nao existem na API, `FromJSonString/ToJSonString` nao estao expostos a camada de eventos e `Refresh` mantem o handler vivo mas nao inclui o SDT no `oparms` — para a diferenca estrutural entre rejeicao na importacao (`src0294`, mecanismo a) e strip silencioso por DCE (mecanismo b), ver subsecao `Mecanismos de descarte de codigo de evento pelo gerador GeneXus`.
- `Anti-pattern`: `<InnerTab>.SelectTab(1)` sozinho, quando a sub-aba 1 ja eh a ativa default, eh no-op no cliente; o par `(2);(1)` eh o que produz o efeito.
- `Procedimento de verificacao no .cs gerado`: apos build, no `<wpName>.cs`, confirmar (1) que o handler do evento existe (busca por `TABCHANGED` ou nome do evento); (2) que o SDT aparece no `oparms` do evento — buscar `setEventMetadata("<EVENTO>", ... "oparms":[...])` e verificar `<NomeAvDoSdt>` no array; (3) que a chamada `ajax_rsp_assign_sdt_attri("..", false, "<NomeAv>", <NomeAv>)` esta no corpo do handler. Handler stripado por DCE significa que falta operacao reconhecivel pelo gerador; SDT ausente do `oparms` significa que o write em membro nao foi detectado como mutacao real.
- `Escopo da evidência`: padrao observado em um `WebPanel` real (`wpGtaConsultaDoGoverno` em Gx_FabricaBrasil), com duas branches por outer tab e familia local de procs helper por tipo (`procVoltaDo<Tipo>QueFoi`); aplicabilidade em `Panel SD` nao foi verificada localmente e deve ser validada caso a caso quando a estrutura de eventos for equivalente.
...
### Mecanismos de descarte de codigo de evento pelo gerador GeneXus

- `Evidência direta`: o gerador pode silenciar codigo de evento por dois caminhos estruturalmente distintos, e o diagnostico exige diferencia-los.
- `Mecanismo (a) — rejeicao na importacao`: a chamada existe na API do `.cs` gerado mas nao esta exposta a camada de eventos GeneXus (caso observado: `ToJSonString`/`FromJSonString` no SDT). O import via MSBuild falha com mensagem `Unknown function '<nome>'` (codigo `src0294` ou similar), `exitCode != 0` e numero de linha/coluna apontando o source do evento; o objeto NAO eh atualizado na KB.
...
- `Regra operacional`: para `Transaction` com sintoma de atributo que nao reflete na UI apos import OK e geracao disponivel, resolver primeiro o `.cs` web por `scripts/Resolve-GeneXusGeneratedCsPath.ps1` (ou wrapper local) a partir de `kb_environment_web_dirs`; em seguida mapear atribuicoes com `scripts/Find-CsAttributeAssignments.ps1` (`-CsPath` absoluto resolvido, `-Attribute`, `-AsJson`) antes de concluir que a rule foi eliminada; distinguir copias sem `AssignAttri` no mesmo metodo e padrao triplet do Specifier (`cascadeOrder` em `if/else if` mutuamente exclusivos). Nao substitui busca por nome de evento/handler em `WebPanel` (mecanismo b).
- `Regra operacional` (quarentena XPZ para `Transaction`): `tripletPattern.cascadeOrder` em `Find-CsAttributeAssignments.ps1` e evidencia de ordem efetiva no `.cs` gerado, nao catalogo geral de modelagem GeneXus. Para o anti-padrao nomeado e a correcao por inversao textual de `Rules`, ver `transaction-attribute-rule-shadowed-by-default-in-cascade` na secao **Anti-padroes nomeados (`Transaction` — motor XPZ)**.

### Diagnostico de codigo gerado truncado por falha de generation

- `Evidência direta`: em build pos-generation, erros `CS1010` (Newline em constante) e `CS1513` (`}` esperada) repetidos no `.cs` referenciado podem indicar truncamento do arquivo gerado, nao erro real no XML do objeto. Caso observado: linha final do `.cs` com menos chars do que o esperado, terminando no meio de uma string literal sem aspa de fechamento e sem newline final.
- `Causa tipica`: o gerador GeneXus interrompeu a geracao no meio da escrita do `.cs` — processo cancelado, exception interna, falta de espaco em disco, IDE travada, etc. O MSBuild compila o arquivo incompleto e os erros C# apontam para a regiao do truncamento.
- `Heuristica para distinguir do erro real no XML`: antes de investigar o XML, conferir no `.cs` referenciado se (i) a linha final termina abruptamente, (ii) ha chamada de funcao em aberto, (iii) ha string literal nao fechada, (iv) o arquivo termina sem newline final e (v) o total de linhas eh inferior ao esperado para o tamanho do objeto. Comparar a regiao com a mesma regiao em outro environment da mesma KB ou em uma KB irma onde a geracao tenha completado: o codigo gerado em torno do mesmo controle deve ser estruturalmente identico.
- `Regra operacional`: antes de mexer no XML, executar primeiro uma regeneracao controlada do objeto ou do conjunto minimo afetado. Em fluxo MSBuild cotidiano, preferir `BuildAll`/`SpecifyGenerate` sem `-ForceRebuild`; `Rebuild All`/`ForceRebuild=true` continua sendo regeneracao ampla e so pode ser usado com o gate de `-AllowWideRebuild` e confirmacao explicita. Se o sintoma persistir apos a regeneracao limpa, ai sim investigar o XML.

### Saida do gerador GeneXus por environment

- `Evidência direta`: para a mesma KB com a mesma versao do objeto, o gerador GeneXus pode produzir codigo `.cs` materialmente diferente entre environments. Caso observado: `Default` (.NET Framework, env CSharpModel) compilando para `net48` e `NETPostgreSQL` (target Core, env equivalente) compilando para `net8.0`; layout do codigo, runtime APIs disponiveis e detalhes de geracao variam.
- `Inferência forte`: um fix de codigo de evento validado em um environment nao se transfere automaticamente para outro; o fix valida o XML, nao a saida final do gerador.
- `Regra operacional`: distinguir **validacao alinhada ao deploy (objetivo A)** — um environment, o que IIS/self-host executa (`deployment_environment_name`, headless com `xpz-msbuild-build`, gate de `web\bin` quando aplicavel) — de **fix completo multi-environment (objetivo B)** — quando a frente exigir fechamento em mais de um generator ativo, nao basta A sozinho.
- `Regra operacional`: objetivo B **nao** exige orquestrador headless nem loop automatico por `kb_environment_names`. Pratica usual e suficiente: apos A bem-sucedido no environment de deploy, **Build na IDE** nos demais environments ativos que a frente ainda cobre (consultar `kb_environment_names` no metadata); conferir compilacao e, se houver duvida, resolver o `.cs` pelo `kb_environment_web_dirs` do metadata (`Resolve-GeneXusGeneratedCsPath.ps1` ou wrapper local) ou, manualmente, pelo output dir declarado em `kb_environment_output_dirs` (`<KbNative>\<OutputDir>\web\`); nao usar o nome do environment como pasta por inferencia. Headless repetido com `-EnvironmentName` e opcional, nao padrao.
- `Regra operacional`: objetivo B e **opt-in por frente** — nao e gate pos-import automatico. Quando o deploy principal passou e o segundo environment e buildado na IDE ao encerrar, falha ai e **rara**; priorizar A + Ponto 2 no fluxo agente/headless. Inventario `kb_environment_count`/`kb_environment_names` e mapeamento `kb_environment_output_dirs`/`kb_environment_web_dirs` ficam em `kb-source-metadata.md` (setup: lista e output dirs declarados pelo usuario em `-KbEnvironmentNames`/`-KbEnvironmentOutputDirs` + validacao MSBuild obrigatoria; sem scan de pastas); build/import nao revarrem a KB nativa para contar environments.
- `Regra operacional`: `CSharpModel\web` e compartilhado entre environments; timestamps frescos ali **nao** provam que o `web\bin` do output de deploy foi atualizado. Nao usar so `Test-GeneXusRuntimeFreshness.ps1` para concluir validacao deploy em KB multi-environment. Validacao deploy pos-import: wrappers com `-PostImportDeployValidation` checam publicacao no `web\bin` resolvido por `kb_environment_web_dirs` (max timestamp de DLL de objeto excluindo runtime GeneXus/System/Microsoft, ou `*.config`); em Core, `GxNetCoreStartup.dll` e so complementar (warning se velho em build incremental); status `compilou-mas-dll-destino-desatualizada` ou exit **49** quando gate ativo sem evidencia de publicacao. O gate decide por **sucesso operacional factual** (MSBuild `exitCode` 0 + marcador de conclusao do build), nao pela string de status: rebaixamento benigno (evento pos-build como sino de fim de build, ruido em stderr) **nao** suprime `-PostImportDeployValidation`; so falha real (Categoria B, reorg, timeout, KB inacessivel — que derrubam o `exitCode`) pula o gate.
- `Regra operacional`: eventos pos-build no stdout (ex.: sino de fim de build, script de deploy `.Bat`) **nao** rebaixam o status cegamente. O wrapper classifica cada evento contra `kb_environment_post_build_event_hashes` do environment ativo em `kb-source-metadata.md`: registrado = esperado (informativo, nao rebaixa); nao registrado = inesperado (rebaixa por cautela); linha comentada (`REM`) e inerte e nunca conta. Sem registro para o environment, rede de seguranca reconhece player de som (`SoundPlayer`/`PlaySync`/`.wav`) como benigno e rebaixa o resto. Registrar eventos legitimos via `xpz-kb-parallel-setup` (`Register-GeneXusKbPostBuildEvents.ps1`, autoria do campo); inspecionar `stdoutSignals.postBuildEventClassification` no diagnostico.
- `Regra operacional`: para diagnostico pontual de `.cs` gerado, nao fazer glob recursivo em `C:\GxModels` nem inferir diretorio por nome parecido de pasta. Usar `scripts\Resolve-GeneXusGeneratedCsPath.ps1` com `-KbPath`, `-ParallelKbRoot` ou `-KbMetadataPath`, `-EnvironmentName` quando necessario e `-ObjectName`; o script le `kb_environment_web_dirs` em `kb-source-metadata.md` e monta `<webDir>\<objeto-lowercase>.cs`. Se o metadata nao tiver `kb_environment_web_dirs` para o environment, bloquear e executar `xpz-kb-parallel-setup` para reconciliar `kb-source-metadata.md`; nao aceitar chute de diretorio como substituto.
...
- `Regra operacional`: ao diagnosticar rule ou atributo `Transaction` no codigo gerado da camada web, resolver primeiro o `.cs` da transacao com `scripts/Resolve-GeneXusGeneratedCsPath.ps1` (ou wrapper local) e entao usar `scripts/Find-CsAttributeAssignments.ps1` no caminho absoluto resolvido apos build ou `SpecifyGenerate` quando o artefato estiver disponivel; consumo complementar documentado em `xpz-msbuild-build/SKILL.md`.
...
- **Sintoma observado**: rule de atribuicao existe no XML, eh aceita pelo Specifier, importa OK e builda OK; em runtime o atributo nao recebe o valor esperado da rule, recebendo o `Default(...)` por proc (frequentemente `0`) ou ficando vazio. Via edicao web, o sintoma tipico e campo de FK derivada continuar vazio depois de tabular do atributo gatilho, mesmo com rule no XML que indicaria preenchimento.
- **Erro do Specifier**: nenhum. A patologia nao aparece como erro de importacao, especificacao, geracao ou compilacao; exige inspecao do `.cs` gerado da camada web.
- **Causa raiz**: `Default(...)` e rule de atribuicao sobre o mesmo atributo podem virar cascata `if`/`else if` mutuamente exclusiva no `.cs` gerado em metodos como `OnLoadActions<KbId>`, `CheckExtendedTable<KbId>`, `Valid_<FKDoLado>` e `GX<n>ASA<ATTR><KbId>`. A ordem textual no XML determina a ordem dos branches; quando o default fica antes da rule, ele pode sombrear a atribuicao pretendida.
- **Correcao idiomatica**: inverter a ordem textual no XML da frente, colocando a assignment rule antes de `Default(...)`. Aplicar via `scripts/Edit-GeneXusXmlSurgical.ps1` com `-DryRun`, ancora literal, `-ExpectedAnchorCount` e baseline de `lastUpdate` quando houver; nao reserializar a `Transaction` inteira. Revalidar por import/build e nova leitura do `.cs`.
...
- **Evidencia**: `confirmado-acervo` na trilha XPZ (`VendaPedido` em pasta paralela apos sync) e `confirmado-build` por inspecao reproduzivel do `.cs` antes/depois.
- **Nota de escopo (quarentena XPZ)**: este conhecimento pertence naturalmente a modelagem GeneXus e a skills de produto como **nexa**. Ele fica documentado aqui porque o diagnostico operacional usa `scripts/Find-CsAttributeAssignments.ps1`, a correcao aprovada acontece no XML da pasta paralela via helper XPZ, e a skill **nexa** nao e contribuivel por esta base. Se **nexa** ou skill equivalente se tornar contribuivel, esta entrada e candidata a migracao; nao generalizar para outros temas semanticos GeneXus sem o mesmo criterio de diagnostico e correcao dentro do escopo XPZ.
...
- `Regra operacional`: quando `Sync-GeneXusXpzToXml.ps1` ou a pre-varredura (`Get-GeneXusImportPackageObjectInventory.ps1` com `-FailOnUnknownTypes`) reportarem GUID de `Object/@type` ausente do catálogo efetivo, tratar como lacuna de catálogo da base XPZ — nao como defeito do XPZ exportado pela KB.
- `Regra operacional`: nao materializar parcialmente nem improvisar pasta `Unknown_*` em `ObjetosDaKbEmXml` sem decisao explicita do usuario e entrada documentada no override local.
- `Regra operacional`: antes de sync full ou primeira materializacao longa, executar inventario do pacote com `-FailOnUnknownTypes`; se bloquear, nao chamar materializacao.
- `Regra operacional`: ao detectar tipo desconhecido, perguntar ao usuario se autoriza consulta recomendavel a skill `nexa` e documentacao oficial GeneXus (`docs.genexus.com`, `wiki.genexus.com`); nao consultar automaticamente sem consentimento.
- `Regra operacional`: registro local paliativo somente em `scripts/gx-object-type-catalog.override.json` na pasta paralela, via `Register-GeneXusObjectTypeCatalogOverride.ps1` com `-UserApproved` apos consentimento explicito; manter `upstreamPending=true` ate o mantenedor atualizar `scripts/gx-object-type-catalog.json` e `01a-catalogo-e-padroes-empiricos.md` em GeneXus-XPZ-Skills.
- `Regra operacional`: override local nao e silencioso — em cada nova sessao na pasta paralela, executar `Test-XpzCatalogOverrideSessionReminder.ps1` e repetir ao usuario que ainda falta alinhar o catálogo compartilhado.
- `Regra operacional`: oferecer `New-GeneXusUnknownTypeMaintainerPrompt.ps1` para gerar prompt copiavel ao mantenedor da base XPZ; nao editar GeneXus-XPZ-Skills a partir da pasta paralela sem troca de contexto explicita.
- `Evidência direta`: motores `Sync-GeneXusXpzToXml.ps1`, inventario, `Test-XpzObjetosDaKbNaming.ps1` e `Build-KbIntelligenceIndex.py` (via `Build-KbIntelligenceIndex.ps1`) aceitam `-ParallelKbRoot` / `-CatalogOverridePath` (ou deteccao automatica de `scripts/gx-object-type-catalog.override.json` a partir de `ObjetosDaKbEmXml`) e usam o mesmo catalogo efetivo (base + override); sync e inventario agregam amostras (`name`, `parent`, `parentType`, snippet XML) em falha ou JSON de triagem.
- `Evidência direta`: `scripts/GeneXusPythonPrerequisite.ps1` e `Build-KbIntelligenceIndex.ps1` exigem Python 3 utilizavel (exit `8`, mensagem `PREREQUISITO AUSENTE`); falha do `.py` propaga saida capturada; molde `Update-KbFromXpz.example.ps1` chama rebuild no mesmo `pwsh` e prefixa que a materializacao XPZ/XML pode ter concluido; **rigor**: sync oficial **nao** termina sem indice regenerado — declarar **fluxo incompleto**, nao declarar sync OK nem autorizar triagem ampla sem indice; ver `README.md`, `08-guia-para-agente-gpt.md` e `xpz-sync`.
