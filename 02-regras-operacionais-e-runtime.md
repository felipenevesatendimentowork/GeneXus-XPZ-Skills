...
- `Regra operacional`: em trilha `MSBuild`, distinguir sempre `sucesso operacional da chamada`, `preview apenas`, `sucesso operacional com falha no pos-processamento` (MSBuild concluiu com `exitCode=0` — campo `msBuildExitCode=0` no JSON — e a evidencia primaria do log bruto esta presente — `__IMPORTED_ITEM__` ou `__EXPORTED_FILE__` mais arquivo XPZ existente —, mas o pos-processamento local do wrapper falhou e o JSON saiu com `postProcessingFailed=true`), `preview apenas com falha no pos-processamento` (analogo ao anterior na fase de preview: MSBuild concluiu com `exitCode=0` — campo `msBuildExitCode=0` no JSON — sem alterar a KB e a evidencia primaria do log bruto esta presente, mas o pos-processamento local do wrapper falhou e o JSON saiu com `postProcessingFailed=true`) e `confirmacao funcional posterior na IDE`. O campo `exitCode` no JSON dos wrappers e o valor classificado pelo wrapper (0/32/41/42/...); o valor bruto da task MSBuild fica preservado em `msBuildExitCode` e em `executionEvidence`, enquanto `blockingReasons` deve concentrar causas acionaveis.
