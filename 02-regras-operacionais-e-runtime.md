...
- `Regra operacional`: antes de declarar fix completo em objeto tocado por multiplos environments ativos da KB, build em CADA environment ativo (`<KbNative>/<EnvName>/`) e conferir o `.cs` gerado em cada um. A validacao em um environment no maximo aumenta a confianca de que o XML esta bem-formado; nao reduz a necessidade de validar nos demais.
- `Regra operacional`: distinguir **validacao alinhada ao deploy** (um environment — o que o IIS/self-host executa; ver `deployment_environment_name` em `kb-source-metadata.md` e `-EnvironmentName` em `xpz-msbuild-build`) de **fix completo multi-environment** (rodada por environment listado no metadata). Inventario de quantos environments existem na KB fica em `kb_environment_count`/`kb_environment_names`, gravados pelo `xpz-kb-parallel-setup` — build/import nao revarrem a pasta nativa da KB para contar environments.
