...
- `Regra operacional` (quarentena XPZ para `Transaction`): `tripletPattern.cascadeOrder` em `Find-CsAttributeAssignments.ps1` e evidencia de ordem efetiva no `.cs` gerado, nao catalogo geral de modelagem GeneXus. Os valores conhecidos `override-then-default-then-fallback` e `override-then-fallback-then-default` devem ser lidos como diagnostico de ramos mutuamente exclusivos no codigo gerado. Se a correcao aprovada for inverter a ordem textual das `Rules` no XML da frente, aplicar o delta com `scripts/Edit-GeneXusXmlSurgical.ps1` (`-DryRun`, ancora literal, `-ExpectedAnchorCount`, baseline de `lastUpdate` quando houver) e validar de novo por import/build e nova leitura do `.cs`; nao reserializar a `Transaction` inteira nem extrapolar a regra para BC ou outros contextos sem evidencia propria.
