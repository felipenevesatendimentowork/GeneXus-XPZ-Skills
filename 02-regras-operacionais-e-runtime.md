...
- `Regra operacional`: para adicionar um botao a `WebPanel`, preferir `scripts/Add-GeneXusButton.ps1`, que insere `<cell>` apos a celula de um controle folha em tabela Flex (com stub de Event e bump de `lastUpdate`, por substituicao cirurgica); em tabela Responsive com `responsiveSizes` preenchido o helper aborta fail-closed (`RESPONSIVE_UNSAFE`) em vez de reescrever o array de breakpoints — nesse caso, gerar o snippet e ajustar `responsiveSizes` manualmente.
