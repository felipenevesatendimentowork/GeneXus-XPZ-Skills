...
- `Anti-pattern`: `&sdt = &sdt`, `&sdt.<membro> = &sdt.<membro>`, `&sdt.<campo> = &sdt.<campo> + ""`, `&sdt.FromJson(&sdt.ToJson())`, `&sdt.FromJSonString(&sdt.ToJSonString(false))` e `Refresh` sozinho nao funcionam — os primeiros sao eliminados como no-op pelo dead-code do gerador (handler stripado do `.cs` sem erro nem warning), `FromJson/ToJson` nao existem na API, `FromJSonString/ToJSonString` nao estao expostos a camada de eventos e `Refresh` mantem o handler vivo mas nao inclui o SDT no `oparms`.
