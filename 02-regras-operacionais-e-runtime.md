...
- **Verificacao**: executar `scripts/Find-CsAttributeAssignments.ps1` antes e depois (`-CsPath` absoluto, `-Attribute`, `-AsJson`). `tripletPattern.cascadeOrder` deve mudar de `override-then-default-then-fallback` para `override-then-fallback-then-default`.
