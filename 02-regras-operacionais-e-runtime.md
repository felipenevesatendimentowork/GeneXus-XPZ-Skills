...
- `Regra operacional`: a montagem do `import_file.xml` a partir de XMLs de objeto deve usar motor estruturado compartilhado, nunca concatenacao textual manual. Para montagem direta com template clonavel, usar `scripts\Build-GeneXusImportFileEnvelope.ps1`, que executa o gate de colisao (`Test-XpzPackageCollision.ps1`) antes de qualquer escrita — colisao de `_nn` aborta o script sem materializar nenhum byte — clona `KMW`/`Source`/`Dependencies`/`ObjectsIdentityMapping` do template, embute cada objeto via `ImportNode` (impossibilitando declaracao XML interna em `<Objects>`) e executa o gate de envelope automaticamente. Para montagem por frente da pasta paralela, usar `scripts\New-XpzImportPackage.ps1` como wrapper do motor `scripts\New-XpzImportPackage.py`, que classifica raizes top-level `Object`/`Attribute`, aplica gate de colisao e pode usar `kb-source-metadata.md` para envelope minimo ou `-TemplatePackagePath` para clonar pacote real comparavel. Em pacote misto/complexo, preferir `-TemplatePackagePath` com export real comparavel; envelope minimo derivado de metadados e apoio operacional para casos simples, nao formato universal.
