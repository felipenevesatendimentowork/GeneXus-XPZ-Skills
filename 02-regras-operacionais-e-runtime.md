...
- `Regra operacional`: os wrappers MSBuild que suportam watcher (`Invoke-GeneXusKbBuildAll.ps1`, `Invoke-GeneXusKbSpecifyGenerate.ps1`, `Test-GeneXusXpzImportPreview.ps1`, `Invoke-GeneXusXpzExport.ps1` e `Invoke-GeneXusXpzImport.ps1`) devem expor o mesmo contrato (`-StartWatcher`, `-MonitorLogPath`, `-WatcherIntervalSeconds`, `-WatcherSilenceThresholdSeconds`) e registrar `watcherContext`/`timing.phases` por meio de `scripts\GeneXusMsBuildWatcherSupport.ps1`; `-StartWatcher` sem `-MonitorLogPath` deve bloquear cedo com exit 46. Em `Invoke-GeneXusKbBuildAll.ps1` e `Invoke-GeneXusKbSpecifyGenerate.ps1`, watcher e fluxo padrao; em preview/export/import, e padrao para execucao longa. Em importacao real de pacote amplo ou com muitos `WorkWithForWeb`, executar sem watcher deve ser tratado como excecao operacional justificada, porque a importacao pode disparar regeneracao dos objetos derivados do pattern e durar tempo significativo sem feedback visual.
...
- `Regra operacional`: quando o usuario ja autorizou importacao real headless na mesma sessao, `Test-GeneXusImportFileEnvelope.ps1` retornou `apto para prosseguir` e o inventario do pacote (passo 6c de `xpz-msbuild-import-export`) passou sem bloqueio de extras, o agente deve executar `Invoke-GeneXusXpzImport.ps1` na mesma rodada com `-StartWatcher` e `-MonitorLogPath`; nao encerrar so pelo envelope apto nem exigir `Test-GeneXusXpzImportPreview.ps1` obrigatorio antes do import nesse cenario.
