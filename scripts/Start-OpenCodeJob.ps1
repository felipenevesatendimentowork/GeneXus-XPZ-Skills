...
.DESCRIPTION
    ...
    <GUID>.stream.jsonl. Retorna imediatamente jobId+pid (nao bloqueia o chamador). Por
    padrao abre Watch-OpenCodeJob.ps1 numa janela visivel para acompanhar ao vivo; use
    -NoWatcher para suprimir.

    Para perguntas curtas (resposta na hora) use Invoke-OpenCode.ps1. Este script e para
    tarefas longas (ex: 3-6 min) que voce quer disparar e acompanhar sem bloquear.
...
#     Remove por arquivo (LiteralPath); jobs em andamento tem arquivos recentes e sao preservados.
try {
...
# 6) Dispara o opencode desanexado (janela oculta, nao espera)
