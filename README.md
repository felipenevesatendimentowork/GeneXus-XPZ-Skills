...
- tratar `kb-source-metadata.md` por autoridade de campo: identidade estavel da KB pertence ao setup/resolvedor da KB nativa local, `KMW` vem de XPZ real ou template comparavel, metadados de materializacao pertencem ao `xpz-sync`, e `last_setup_audit_run_at` pertence ao setup/auditoria (`xpz-kb-parallel-setup`)
- tratar os campos de environment/deploy (`deployment_environment_name`, `deployment_hosting_kind`, `kb_environment_count`, `kb_environment_names`) como autoridade do setup/auditoria (`xpz-kb-parallel-setup`): os nomes vêm de lista declarada pelo usuário em `-KbEnvironmentNames` e validação MSBuild (`SetActiveEnvironment`) obrigatória, não de scan de pastas da KB nativa; `metadata/deploy` diferente de `OK` bloqueia estado limpo até reconciliação. Em validação pós-import, `Invoke-GeneXusKbBuildAll.ps1` e `Invoke-GeneXusKbSpecifyGenerate.ps1` devem usar `-ParallelKbRoot`/`-KbMetadataPath` e, quando aplicável, `-PostImportDeployValidation` para checar publicação em `<KbNative>\<EnvDeploy>\web\bin` por DLL de objeto ou `*.config`; `GxNetCoreStartup.dll` sozinho não prova deploy atualizado
- apos auditoria de setup bem-sucedida com `GATE_OK`, gravar `last_setup_audit_run_at` na mesma sessao via `scripts/Set-XpzSetupAuditTimestamp.ps1` (wrapper local `Set-*KbSetupAuditTimestamp.ps1`) quando `Test-*KbSetupFreshness.ps1` tiver exigido auditoria completa por campo ausente ou defasado; adiar so com recusa ou adiamento explicito do usuario
...
- quando o sync XPZ/XML atualizar `kb-source-metadata.md` com `-KbMetadataPath`, o motor `scripts/Sync-GeneXusXpzToXml.ps1` (via `scripts/XpzKbSourceMetadataEditSupport.ps1`) faz atualizacao **cirurgica** dos campos de materializacao, preservando `last_setup_audit_run_at`, demais frontmatter fora do escopo e EOL dominante (`scripts/XpzTextFileEolSupport.ps1`)
...
- tratar `kb-source-metadata.md` por autoridad de campo: la identidad estable de la KB pertenece al setup/resolutor de la KB nativa local, `KMW` viene de un XPZ real o plantilla comparable, los metadatos de materializacion pertenecen a `xpz-sync`, y `last_setup_audit_run_at` pertenece al setup/auditoria (`xpz-kb-parallel-setup`)
- tratar los campos de environment/deploy (`deployment_environment_name`, `deployment_hosting_kind`, `kb_environment_count`, `kb_environment_names`) como autoridad del setup/auditoria (`xpz-kb-parallel-setup`): los nombres vienen de una lista declarada por el usuario en `-KbEnvironmentNames` y validacion MSBuild (`SetActiveEnvironment`) obligatoria, no de scan de carpetas de la KB nativa; `metadata/deploy` distinto de `OK` bloquea el estado limpio hasta la reconciliacion. En validacion post-import, `Invoke-GeneXusKbBuildAll.ps1` e `Invoke-GeneXusKbSpecifyGenerate.ps1` deben usar `-ParallelKbRoot`/`-KbMetadataPath` y, cuando aplique, `-PostImportDeployValidation` para verificar publicacion en `<KbNative>\<EnvDeploy>\web\bin` por DLL de objeto o `*.config`; `GxNetCoreStartup.dll` solo no prueba deploy actualizado
- despues de una auditoria de setup exitosa con `GATE_OK`, grabar `last_setup_audit_run_at` en la misma sesion via `scripts/Set-XpzSetupAuditTimestamp.ps1` (wrapper local `Set-*KbSetupAuditTimestamp.ps1`) cuando `Test-*KbSetupFreshness.ps1` haya exigido auditoria completa por campo ausente o desfasado; posponer solo con rechazo o aplazamiento explicito del usuario
...
- cuando el sync XPZ/XML actualice `kb-source-metadata.md` con `-KbMetadataPath`, el motor `scripts/Sync-GeneXusXpzToXml.ps1` (via `scripts/XpzKbSourceMetadataEditSupport.ps1`) hace actualizacion **quirurgica** de los campos de materializacion, preservando `last_setup_audit_run_at`, el resto del frontmatter fuera de alcance y el EOL dominante (`scripts/XpzTextFileEolSupport.ps1`)
...
- when using `New-XpzImportPackage.ps1` with `-AcervoPath <ObjetosDaKbEmXml>`, the 9-FD front-acervo drift gate (`Test-GeneXusFrontAcervoDrift.ps1`) runs before packaging and blocks XMLs in the front whose `lastUpdate` is older than the matching file in the acervo; without `-AcervoPath`, packaging proceeds without drift checking (previous behavior unchanged)
...
- treat `kb-source-metadata.md` by field authority: stable KB identity belongs to setup/the local native KB resolver, `KMW` comes from a real XPZ or comparable template, materialization metadata belongs to `xpz-sync`, and `last_setup_audit_run_at` belongs to setup/audit (`xpz-kb-parallel-setup`)
- treat the environment/deploy fields (`deployment_environment_name`, `deployment_hosting_kind`, `kb_environment_count`, `kb_environment_names`) as setup/audit authority (`xpz-kb-parallel-setup`): names come from a user-declared `-KbEnvironmentNames` list and mandatory MSBuild (`SetActiveEnvironment`) validation, not from scanning native KB folders; `metadata/deploy` different from `OK` blocks a clean state until reconciliation. In post-import validation, `Invoke-GeneXusKbBuildAll.ps1` and `Invoke-GeneXusKbSpecifyGenerate.ps1` must use `-ParallelKbRoot`/`-KbMetadataPath` and, when applicable, `-PostImportDeployValidation` to check publication in `<KbNative>\<EnvDeploy>\web\bin` by object DLL or `*.config`; `GxNetCoreStartup.dll` alone does not prove an updated deploy
- after a successful setup audit with `GATE_OK`, write `last_setup_audit_run_at` in the same session via `scripts/Set-XpzSetupAuditTimestamp.ps1` (local wrapper `Set-*KbSetupAuditTimestamp.ps1`) when `Test-*KbSetupFreshness.ps1` required a full audit for a missing or stale field; defer only with explicit user refusal or postponement
...
- when XPZ/XML sync updates `kb-source-metadata.md` with `-KbMetadataPath`, the engine `scripts/Sync-GeneXusXpzToXml.ps1` (via `scripts/XpzKbSourceMetadataEditSupport.ps1`) performs a **surgical** update of materialization fields, preserving `last_setup_audit_run_at`, other out-of-scope frontmatter, and dominant EOL (`scripts/XpzTextFileEolSupport.ps1`)
