...
- para diagnostico compacto de Panel e WebPanel, preferir `scripts/Get-GeneXusObjectSummary.ps1` (com bloco `webpanel` para WebPanel classico) e, para Panel, `scripts/Compare-GeneXusPanelShape.ps1`, antes de despejar XML/CDATA completo; para adicionar botao a WebPanel, preferir `scripts/Add-GeneXusButton.ps1` (insercao cirurgica antes ou apos um controle folha via `-BeforeControlName`/`-AfterControlName`, fail-closed em tabela Responsive com breakpoints)
