...
- `xpz-llm-delegate`: delegação de tarefas menores ou segunda opinião a um LLM secundário via opencode ou Codex (`codex exec`, GPT-5.5), com classificação local/externo determinística e gate de confidencialidade por KB (chave de destino, nunca o adapter); acionamento sempre humano
...
- `xpz-llm-delegate`: delegación de tareas menores o segunda opinión a un LLM secundario vía opencode o Codex (`codex exec`, GPT-5.5), con clasificación local/externo determinística y gate de confidencialidad por KB (clave de destino, nunca el adapter); activación siempre humana
...
- `xpz-llm-delegate`: delegation of smaller tasks or second opinion to a secondary LLM via opencode or Codex (`codex exec`, GPT-5.5), with deterministic local/external classification and per-KB confidentiality gate (destination key, never the adapter); always human-triggered
