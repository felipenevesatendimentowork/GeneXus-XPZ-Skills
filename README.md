...
- `xpz-llm-delegate`: delegação de tarefas menores ou segunda opinião a um LLM secundário via opencode, Codex (`codex exec`), Claude Code (`claude -p`, Opus 4.8), GitHub Copilot CLI (`copilot -p`) ou Gemini CLI (`gemini -p`), com classificação local/externo determinística e gate de confidencialidade por KB (chave de destino, nunca o adapter); acionamento sempre humano
...
- `xpz-llm-delegate`: delegation of smaller tasks or second opinion to a secondary LLM via opencode, Codex (`codex exec`), Claude Code (`claude -p`, Opus 4.8), GitHub Copilot CLI (`copilot -p`), or Gemini CLI (`gemini -p`), with deterministic local/external classification and per-KB confidentiality gate (destination key, never the adapter); always human-triggered
