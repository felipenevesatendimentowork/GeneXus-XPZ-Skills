...
- em `BuildAll` via MSBuild, usar watcher (`-StartWatcher` com `-MonitorLogPath`) como fluxo padrao; `BuildAll` sem watcher visivel exige justificativa operacional explicita e registro da ausencia por `watcherContext.watcherLaunched=false`
...
- en `BuildAll` vía MSBuild, usar watcher (`-StartWatcher` con `-MonitorLogPath`) como flujo estándar; `BuildAll` sin watcher visible exige justificación operativa explícita y registro de la ausencia por `watcherContext.watcherLaunched=false`
...
- for MSBuild `BuildAll`, use the watcher (`-StartWatcher` with `-MonitorLogPath`) as the standard flow; `BuildAll` without a visible watcher requires explicit operational justification and absence recorded through `watcherContext.watcherLaunched=false`
