...
**Advertência sobre exportação parcial com lista explícita:** mesmo quando `Objects`/`ObjectList` nomeia objetos concretos, o GeneXus pode incluir no `.xpz` **objetos adicionais** (dependências, referências, módulos organizacionais) e **atributos top-level** (arrasto de base quando a lista não inclui `Transaction`) conforme parâmetros da task (`DependencyType`, `ReferenceType`) e comportamento padrão. **Não** concluir que o pacote coincide com a lista nominal **sem** inventariar o artefato. O motor `scripts/Get-GeneXusImportPackageObjectInventory.ps1` aceita `import_file.xml`, XML com raiz `<ExportFile>` e `.xpz`; confronta delta via `-DeclaredDeltaPath` ou `-DeclaredDeltaItems` (mesmo formato `Tipo:Nome` de `-ObjectList`, separador `;` ou linha); detecta módulos via `scripts/gx-system-modules.txt`, ExternalObjects de plataforma via `scripts/gx-system-external-objects.txt` e emite `attributes-top-level-em-export-cirurgico` em export seletiva sem `Transaction` na lista. Após export headless, `scripts/Invoke-GeneXusXpzExport.ps1` embute `packageInventory` resumido no diagnóstico, grava `package-inventory.json` e classifica `operationalSubState`; linhas `error :` no stdout viram `exportErrors` no top-level do `export.json` via `Read-MsBuildImportSignals.ps1 -Stage export` — **não** tratar `exitCode=0` com `exportErrors` não vazio como conclusão limpa. A skill `xpz-msbuild-import-export` documenta sub-estados, checklist e o anti-padrão de contar a lista nominal no lugar do pacote.
