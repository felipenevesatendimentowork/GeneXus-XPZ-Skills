...
O **recibo mínimo** é obrigatório para usar o rótulo `revisão por pares` na resposta ao humano: arquivos metodológicos lidos, manuscrito/prompt enviado, revisores efetivamente consultados, família de cada revisor, resultado do piso de diversidade, veredito de cada revisor, o **estado da vN+1** (`vNextState`: `notProduced` | `pendingResubmission` | `resubmitted` | `resubmissionDeclinedByHuman` — sempre presente, inclusive `notProduced`, para que o campo não fique silenciosamente ausente) e, quando a montagem usar `xpz-llm-delegate`, o `receiptAddendum` do closeout mecânico. Com `preferred-reviewers.json`, o recibo inclui o estado de todos os preferidos da rodada, inclusive os não consultados. Um retorno **off-task** (revisor não opinou) é `noResponse`, **não** família efetivamente consultada (ver `## Papéis`). Se `Resolve-LlmDelegatePeerReviewCloseout.ps1` devolver `closeoutReady=false` (inclusive por `vnext-pending-resubmission` ou `vnext-resubmission-decline-unaudited`), o orquestrador apresenta o `requiredUserPrompt` e não encerra a rodada como concluída. O `vNextState` é à prova de **silêncio**, não de **fabricação**: registra de forma auditável o que o agente declara, mas não prova a decisão humana.
