...
O **recibo mínimo** é obrigatório para usar o rótulo `revisão por pares` na resposta ao humano: arquivos metodológicos lidos, manuscrito/prompt enviado, revisores efetivamente consultados, família de cada revisor, resultado do piso de diversidade, veredito de cada revisor e, quando a montagem usar `xpz-llm-delegate`, o `receiptAddendum` do closeout mecânico. Com `preferred-reviewers.json`, o recibo inclui o estado de todos os preferidos da rodada, inclusive os não consultados. Se `Resolve-LlmDelegatePeerReviewCloseout.ps1` devolver `closeoutReady=false`, o orquestrador apresenta o `requiredUserPrompt` e não encerra a rodada como concluída.
