...
- **Independência:** todos os revisores recebem o **mesmo manuscrito** e um **prompt mínimo**, sem enriquecimento que contamine a revisão com o que o autor já sabe ou fez.
- **Diversidade de modelo:** preferir revisores de **famílias diferentes** (cegos diferentes da do autor). Inclui subagentes locais e LLMs secundários via [`xpz-llm-delegate`](xpz-llm-delegate/SKILL.md).
...
- **Convergência é o pronto:** encerra quando o painel diverso **inteiro** converge em **zero gaps sobre a versão final**. Passe parcial — inclusive o do dissidente sozinho, ou de revisores que aprovaram uma versão anterior — **não** conta.
...
- **(a) Montagem do painel** — disparar os revisores diversos (via [`xpz-llm-delegate`](xpz-llm-delegate/SKILL.md) + subagentes), cada um com o manuscrito + o prompt mínimo, e **colher os vereditos**. O agente **pode** fazê-lo, como mãos do humano, **sob acionamento humano**.
...
Princípio: **famílias distintas da do autor**, cegos independentes, e **modelos fortes**. Vale a política de modelos do [`README.md`](README.md) ("modelos de linguagem"): preferir os de forte aderência a instruções e **não** recomendar os fracos, que tendem a passar batido em gaps reais — observado na prática, em que modelos fracos aprovam o que um modelo forte reprova. Descobrir os modelos e backends disponíveis na máquina conforme a skill [`xpz-llm-delegate`](xpz-llm-delegate/SKILL.md), em vez de re-sondar a cada uso. O princípio e a política do `README` prevalecem sobre qualquer lista fixa de modelos.
