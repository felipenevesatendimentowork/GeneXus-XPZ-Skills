...
**Origem:** sessão de alinhamento do watcher MSBuild e revisão pré-push em 2026-05-22. Frente watcher fechada nos commits `e55a150` … `68860f5` (`GeneXusMsBuildWatcherSupport.ps1`, docs e escopo no `10-base`). Durante a revisão ficou explícito que `exit 46` e outros códigos estão espalhados em prosa (parâmetros, `02`, skills) sem índice único — o que gera ambiguidade para agentes e duplicação editorial.

**Anti-duplicata:** buscado em `999`/`998` por `catálogo.*exit|exit.*catalog|msbuild-exit-codes` em 2026-05-22, sem entrada equivalente. A entrada «Parsing estruturado de log de build» trata erros *dentro* do log; esta trata o *número de saída do processo / `exitCode` no JSON*.

### Problema concreto que motiva a ideia

Hoje coexistem vocabulários parcialmente documentados:

| Família | Catálogo hoje |
|---------|----------------|
| `Test-GeneXusMsBuildSetup.ps1` | Completo no `10-base`: `0`, `10`–`16`, `90` |
| Import / preview / export | Fragmentado: `0`, `31`–`34`, `41`–`42`, `46`, `90` em código; na doc só menção genérica «0/32/41/42/…» |
| Build / specify | `46`, `47`, `90` + classificação rica por `status` (`compilou limpo`, …) onde o `exitCode` pode ser outro valor (ex.: `45` na matriz empírica) |

O **`exit 46`** concentra várias políticas distintas (watcher sem `-MonitorLogPath`, `ForceRebuild` sem `-AllowWideRebuild`, `-ConfirmReorg` sem `-AllowReorg`, combinações inválidas de switches). O terminal mostra só `46`; a causa real está em `summary`, `blockingReasons` e `requestedContext` — mas isso não está indexado.

Efeito prático: agente infere causa errada; documentação repete «exit 46» em bullets de parâmetros com redação inconsistente (ex.: `StartWatcher` vs `MonitorLogPath`).

### Decisões já fechadas (não reabrir na execução)

1. **Fonte estruturada canônica:** arquivo JSON versionado no repositório (não só tabela Markdown).
2. **Modelo híbrido:** JSON = índice machine-readable; `10-base-operacional-msbuild-headless.md` = regras de interpretação (`executionEvidence.msBuildExitCode` vs `exitCode` classificado, `postProcessingFailed`, etc.) + link para o JSON; `02` e `08` = ponteiro curto; skills MSBuild = subconjunto ou link, **sem** copiar a tabela inteira.
3. **Primeira fase documental:** não obrigar wrappers a importar o JSON em runtime; opcional script de verificação em `scripts/` na fase 2.
4. **Não renumerar códigos** nesta frente (ex.: fatiar `46` em `46a/46b`) — documentar variantes no JSON; mudança numérica seria breaking change.
5. **Anexo obrigatório no JSON** para `exit: 46` com array `causes[]` e `disambiguationRequired: true`.

### Caminho proposto do artefato

- **Arquivo:** `scripts/msbuild-exit-codes.catalog.json`
- **Schema:** campo top-level `schemaVersion` (iniciar em `1`), `lastUpdated` (ISO date), `legend`, `codes[]`, opcional `families.probe` apontando para o bloco já existente do setup (`0`, `10`–`16`, `90`) sem duplicar texto longo — usar `reference: "10-base#Test-GeneXusMsBuildSetup"` ou repetir só os números com `documentedIn`.

### Esboço de schema (v1 mínima)
...
|-------|-----|
| `exit` | número inteiro |
| `category` | `probe` \| `politica` \| `cancelamento` \| `msbuild` \| `artefato` \| `suporte` \| `ferramenta` \| `classificado-build` |
| `summary` | uma linha em português |
| `wrappers` | lista de basenames `.ps1` ou `["*"]` quando transversal |
| `stage` | ex.: `pre-build`, `task-support`, `post-msbuild` (opcional) |
| `disambiguationRequired` | boolean; **true** para 46 |
| `causes` | só quando `disambiguationRequired`; cada causa: `id`, `condition`, `wrappers?`, `jsonHints[]` |
| `variants` | quando o mesmo número tiver significados por contexto (candidato: `32`) |
| `documentedIn` | links internos `10-base`, skill, `02` |

**Legenda obrigatória em `legend`:** diferenciar `exitCode` (classificado + exit do processo) de `executionEvidence.msBuildExitCode` (bruto MSBuild).

### Inventário mecânico prévio (executar na sessão limpa)

Rodar no repositório e preencher o JSON a partir do resultado (não confiar só em memória de chat):

```text
rg "exit \d+" scripts/*.ps1 scripts/Test-GeneXus*.ps1
rg "exitCode\s*=\s*\d+" scripts/
rg "function Get-(Import|Export|Preview|Operation).*ExitCode" scripts/
```

Wrappers MSBuild prioritários:
...
- `Open-GeneXusKbHeadless.ps1`, `Get-GeneXusKbProperty.ps1`, `Test-GeneXusKbConsistency.ps1` (escopo e códigos próprios — incluir se emitirem `exitCode` documentável)
- `Test-GeneXusMsBuildSetup.ps1` (referenciar bloco existente no `10-base`, não reescrever)

**Sementes já conhecidas (validar no inventário):**

| exit | Uso preliminar |
|------|----------------|
| `0` | Sucesso classificado |
| `10`–`16`, `90` | Probe setup (já no `10-base`) |
| `31` | Import: MSBuild falhou (`Get-ImportExitCode`) |
| `32` | Import: update file ausente **ou** processo em `stage=task-support` — **candidato a `variants[]`** |
| `34` | Export: `task-support` |
| `41` | Export: MSBuild ≠ 0 |
| `42` | Export: XPZ não gerado |
| `46` | Política (anexo `causes`) |
| `47` | Cancelamento: frase de confirmação wide rebuild/reorg não aceita (build/specify) |
| `90` | Falha interna do wrapper |

**Causas iniciais para anexo `46` (validar em cada wrapper):**

- `-StartWatcher` sem `-MonitorLogPath`
- `-ForceRebuild` / wide rebuild sem `-AllowWideRebuild` ou confirmação inválida (distinguir `46` bloqueio vs `47` cancelamento)
- `-ConfirmWideRebuild` sem `-AllowWideRebuild`
- `-ConfirmReorg` sem `-AllowReorg`

### Plano de execução (sessão limpa — checklist)

**Fase 0 — Contexto**

- [ ] Ler `README.md`, `AGENTS.md`, `02-regras-operacionais-e-runtime.md` (regras MSBuild + watcher).
- [ ] Ler `10-base-operacional-msbuild-headless.md` (contrato JSON, probe `10`–`16`, parágrafos watcher).
- [ ] Ler `xpz-msbuild-build/SKILL.md` e `xpz-msbuild-import-export/SKILL.md` (trechos `exitCode` / política).
- [ ] `git log origin/main..HEAD --oneline` — confirmar frente watcher já em `main` local.

**Fase 1 — Artefato JSON**

- [ ] Criar `scripts/msbuild-exit-codes.catalog.json` com `schemaVersion: 1`.
- [ ] Preencher `codes[]` a partir do inventário mecânico.
- [ ] Implementar anexo `causes` completo para `exit: 46`.
- [ ] Marcar `variants` onde o mesmo número tiver contextos distintos (mínimo: investigar `32`).

**Fase 2 — Documentação (edições pequenas, ancoradas)**

- [ ] `10-base`: nova seção «Catálogo de códigos de saída» — legenda + «fonte canônica: `scripts/msbuild-exit-codes.catalog.json`» + regra «46 exige desambiguação»; **não** colar tabela gigante em Markdown.
- [ ] `02`: uma regra operacional apontando para o JSON + desambiguação do 46.
- [ ] `08`: uma linha na escada MSBuild («não inferir causa só pelo exit»).
- [ ] `09-inventario-e-rastreabilidade-publica.md`: linha de evidência do novo catálogo.
- [ ] Skills `xpz-msbuild-build` / `xpz-msbuild-import-export`: onde hoje há listas longas de «exit 46» em bullets de parâmetro, enxugar para «ver catálogo em `scripts/msbuild-exit-codes.catalog.json`» mantendo só o essencial operacional.

**Fase 3 — Verificação (opcional mas recomendada)**

- [ ] Novo script `scripts/Test-MsBuildExitCodesCatalog.ps1` (parse JSON + `rg` de `exit N` nos wrappers listados no catálogo; falhar se houver código no script ausente do JSON, salvo lista `allowedUndocumented` temporária).
- [ ] Invocar parse via `scripts/Test-PsScriptsParse.ps1` se criar script novo.
- [ ] Rotina pré-push: busca cruzada `exit 46`, `msbuild-exit-codes`, coerência `02`/`08`/`10`/skills.

**Fase 4 — Fechamento**

- [ ] Mover esta entrada para `historico/IdeiasImplementadas_202605.md` com hash do commit e resumo.
- [ ] Avaliar `GeneXus-XPZ-PrivateMap` — N/A salvo se catálogo virar molde publicável (não é o caso).

### Fora de escopo desta frente

- Parsing/agrupamento de erros **dentro** do log de build (ver entrada «Parsing estruturado de log de build» abaixo).
- Modo `-Async` / long-poll nos wrappers.
- Alterar valores numéricos de exit nos `.ps1`.
- Tradução ES/EN do JSON (conteúdo técnico pode ficar PT na v1; README trilíngue só se houver parágrafo novo no `README` — preferir não duplicar catálogo no README).

### Relacionado

- Frente watcher: `GeneXusMsBuildWatcherSupport.ps1`, commits `e55a150`–`68860f5`.
- `02-regras-operacionais-e-runtime.md` (regra watcher + `ForceRebuild` exit 46).
- Entrada «Parsing estruturado de log de build» — complementar, não substituta.

### Perguntas residuais (resolver na sessão se o inventário contradizer)

- `32` deve virar dois códigos documentais (`variants`) ou dois exits distintos no código (só com aprovação explícita de breaking change)?
- Build: quais `exitCode` classificados além de `0`/`45`/`46`/`47`/`90` precisam de linhas próprias vs nota «ver classificação por `status` na skill build»?
- Incluir `Open-GeneXusKbHeadless.ps1` / `Get-GeneXusKbProperty.ps1` no mesmo JSON ou sub-objeto `families.headless`?
