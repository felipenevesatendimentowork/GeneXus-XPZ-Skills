...
**Cross-validação pelo full sync do dev (mesmo episódio):** GUID idêntico nos dois arquivos confirma rename (não delete+add); full export limpo (485 objetos, exitCode=0, sem Categoria B). Dois aprendizados: (a) **detectar** o regime destrutivo é barato — o `-FullSnapshot` acusou o nome antigo como `Extra=1` (reconciliação de full export sinaliza rename/remoção de graça; no incremental, `git diff` mostra o arquivo deletado/criado). O difícil não é detectar, é **enumerar consumidores + veredito** (grep ambíguo, build autoritativo). (b) Gap de tooling **identificado na época e já resolvido**: a reconciliação de resíduo de rename tratava `<Object>` e ignorava `<Attribute>`, deixando o arquivo de nome antigo como resíduo no acervo. **Resolvido** pela frente Sync GUID-aware (`Resolve-GuidAwareRenames` em `scripts/Sync-GeneXusXpzToXml.ps1`), que sob `-FullSnapshot` reconcilia o rename pela identidade `guid` cobrindo **`Attribute` e `Object`** (renomeia o arquivo existente em vez de deixar resíduo); ver `CHANGELOG.md` e `xpz-sync/SKILL.md`.
...
## Gate fail-fast de `-LogPath` (rejeitar diretório) nos wrappers MSBuild

**Importância:** média (build longo desperdiçado + diagnóstico enganoso — o wrapper retorna exit `90` "falha operacional" quando o build na verdade concluiu; workaround = passar caminho de arquivo correto)
**Maturidade:** pronta para implementar (caso real + decisões fechadas por 3 revisões; falta executar)

**Origem:** prompt de outro agente a partir de uso real (frente OperacaoItem / Título Intermediário na KB FabricaBrasil18), 2026-06-13. Plano revisado por 3 análises independentes convergentes: verificação própria + subagente Opus + Codex/GPT-5.5. (O painel opencode de 4 modelos não rodou por bug de infra do opencode — ver `reference_opencode_db_migration_trava`; irrelevante ao mérito.)

### Problema concreto

Em `scripts/Invoke-GeneXusKbBuildAll.ps1` o `-LogPath` é resolvido cedo (`$resolvedLogPath = Get-FullPathSafe`, ~linha 679) mas **não há validação de que aponta para um arquivo**. Se o chamador passa por engano um **diretório existente** como `-LogPath`, o wrapper abre a KB e roda o BuildAll **inteiro**; só no fim, ao gravar o JSON via `Write-JsonLog` → `[System.IO.File]::WriteAllText(<diretorio>, ...)`, a escrita falha (Access denied) e o wrapper cai no catch e retorna **exit 90**, parecendo falha de build quando o build concluiu sem erro. O guard atual em volta da escrita só checa o **diretório-pai** (`Test-Path -LiteralPath $parent -PathType Container`) — um diretório existente passa nesse gate.

### Decisões fechadas (3 revisões convergiram)

- **Descoberta-chave (refuta a 1ª proposta de "terceirizar ao probe"):** o probe `Test-GeneXusMsBuildSetup.ps1` (função `Validate-LogPath`, exit `14`) **não** valida o `-LogPath` do wrapper — os wrappers chamam o probe com um LogPath **interno** (`probe-stage.json`; ver `Invoke-GeneXusKbBuildAll.ps1` ~379 `LogPath=$ProbeLogPath`, ~1221, ~1223). Logo o gate **tem de rodar no próprio wrapper** com o `-LogPath` real; não dá para centralizar no probe.
- **Exit code NOVO `50`** (categoria nova, ex.: `parametro-invalido`), **não** reusar `46` (categoria `politica`/segurança, já multi-causa) nem `14` (família `probe`). `50`/`51` estão livres no `scripts/msbuild-exit-codes.catalog.json`.
- **Função pura compartilhada nova**, escopo mínimo ("o `-LogPath` resolvido é um diretório existente?"), chamada **cedo** em cada wrapper, **antes** de abrir a KB / registrar tarefa; emite o JSON de bloqueio **só no stdout** (sem `Write-JsonLog`, para não tentar gravar no `-LogPath` inválido). **Não** mexer na escrita JSON nem substituir os helpers locais (`Get-FullPathSafe`/`Test-IsUnderProgramFilesX86`/`Write-JsonLog`) que cada wrapper redefine.
- **Condição exata:** `Test-Path -LiteralPath $resolvedLogPath -PathType Container`. **Nunca** `-not (... -PathType Leaf)` — isso quebraria o caso legítimo de arquivo-a-criar (`-LogPath` ainda inexistente com pai válido).
- **Wrappers afetados (10):** `Invoke-GeneXusKbBuildAll`, `Invoke-GeneXusXpzImport`, `Get-GeneXusKbProperty`, `Start-GeneXusKbBuildDetached` (nuance: ~linha 454 ele **cria** o diretório-pai do log/sentinela — o gate deve bloquear "LogPath é diretório existente" mas **permitir** arquivo-a-criar), `Invoke-GeneXusXpzImportThenBuild` (tem `Write-JsonLogIfSafe` que só bloqueia Program Files), `Invoke-GeneXusKbSpecifyGenerate`, `Invoke-GeneXusXpzExport`, `Test-GeneXusXpzImportPreview`, `Open-GeneXusKbHeadless`, `Test-GeneXusKbConsistency`. (Os 2 últimos foram achados pelo Codex; mesmo padrão probe-interno + gravação posterior em `$resolvedLogPath`.)
- **Manter** a política de nunca gravar sob `Program Files (x86)`. **Não regredir** quem passa arquivo válido (inclusive arquivo-a-criar com pai válido / criação de pai já suportada).

### Paridade obrigatória (não esquecer)

- `scripts/msbuild-exit-codes.catalog.json`: nova entrada `50`. **E** ampliar a lista fixa de wrappers do teste `scripts/Test-MsBuildExitCodesCatalog.ps1` (é só **reverso** wrapper→catálogo e hoje **não** cobre `Start-GeneXusKbBuildDetached` nem `Invoke-GeneXusXpzImportThenBuild`).
- `09-inventario-e-rastreabilidade-publica.md` (~linha 50 enumera as faixas de exit code) e a régua de `13-revisao-pre-push.md` (exit codes como conjunto enumerado).
- SKILL.md de `xpz-msbuild-build` e `xpz-msbuild-import-export` + **corrigir o exemplo** em `xpz-msbuild-import-export/SKILL.md` (~linha 476): hoje mostra `-LogPath "<pasta-artefatos>/import"` com `-MonitorLogPath "<pasta-artefatos>/import/watcher.log"` logo abaixo — ensina `-LogPath` apontando para o que parece um **diretório**, o que a nova validação contradiz.
- `10-base-operacional-msbuild-headless.md`; README trilíngue (PT/ES/EN) se a regra virar geral; `CHANGELOG.md` (Unreleased).

### Relacionado

- `scripts/Invoke-GeneXusKbBuildAll.ps1`, `scripts/Get-GeneXusKbProperty.ps1` (caminho de sucesso ~557 grava sem nem o guard de pai), `scripts/Start-GeneXusKbBuildDetached.ps1`, `scripts/msbuild-exit-codes.catalog.json`, `scripts/Test-GeneXusMsBuildSetup.ps1` (`Validate-LogPath`).

## Catalogar o anti-padrão `src0239` (regra `Default` com condicional) em Transaction

**Importância:** baixa (anti-padrão de modelagem; evita um erro de import recorrente, mas dispensável — o Specifier já barra com mensagem clara)
**Maturidade:** pronta para implementar (reprodução real confirmada + lar e formato fechados)

**Origem:** prompt de outro agente, 2026-06-13. **Reprodução real `confirmado-import`** na própria máquina: `Temp/xpz-msbuild-import-export/gx-import-real-.../msbuild.import.signals.json` — `src0239: 'Default' rule does not support a conditional expression. (Transaction 'OperacaoItem' Rules, Linha: 116, Char: 1)`, `importTaskSuccess=false`, KB FabricaBrasil18 / environment NETPostgreSQL. (Busca por `src0239` no repo versionado dá vazio porque `Temp/` é gitignored.)

### Problema concreto

Uma regra de Transaction na forma `Default(<Atributo>, <expressão>)` **não aceita** cláusula condicional. Escrever `Default(OperacaoItemTituloIntermediarioEmpresaId, procEmpresaPessoa()) if not OperacaoItemTituloIntermediarioId.IsEmpty();` faz o specify/import falhar com `src0239`. O anti-padrão **não está catalogado**.

### Decisões fechadas

- **Corpo nomeado no `02-regras-operacionais-e-runtime.md`**, na seção "Anti-padroes nomeados (`Transaction` — motor XPZ)" (~1418-1440), no **mesmo formato das irmãs** (`src0056`/`spc0150`): Sintoma observado / Erro exato (Specifier) **verbatim** / Causa raiz / Correção idiomática, rótulo `confirmado-import`.
- **Ponteiro curto** no Catalog 3 ("Symptom → likely cause → idiom") de `xpz-builder/responsibilities-by-type/transaction.md` (~linha 83-91) apontando para o `02` — seguindo o precedente de `transaction-attribute-rule-shadowed-by-default-in-cascade` (corpo no `02`, ponteiro no satélite).
- **Cross-ref à seção "Cascade ordering"** (Default vs assignment) como **tema relacionado mas mecanismo distinto**: `src0239` é **rejeição no import** (o Specifier barra antes); a cascata é **sombreamento em runtime** com import/build limpos. **Não fundir** (o `02:1416` já adverte "não confundir").
- **Semântica (cuidado para não errar):** a correção "manter o `Default` incondicional e mover a condição para uma regra de assignment separada" **não é equivalente** ao `Default` condicional pretendido — os gatilhos diferem (`Default` = valor inicial quando o atributo está vazio; assignment = avaliação de rule). Documentar como forma idiomática **sem afirmar equivalência de runtime**; a semântica correta de cada uma é conhecimento de modelagem GeneXus (fronteira **nexa**), como nas irmãs.
- `CHANGELOG.md` (Unreleased).

### Relacionado

- `02-regras-operacionais-e-runtime.md` (seção "Anti-padroes nomeados (`Transaction` — motor XPZ)"), `xpz-builder/responsibilities-by-type/transaction.md` (Catalog 3 + seção "Cascade ordering").
