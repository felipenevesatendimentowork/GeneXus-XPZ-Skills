...
## URGENTE — Reduzir as variações de chamada dos adapters de delegação que disparam prompt de autorização

- **Importância** — alta (atrito operacional recorrente: está atrasando e incomodando o usuário em sessões reais; cada prompt desnecessário prende o fluxo). O gargalo **não é falta de script** — são duas causas: (1) o **prompt passado como argv entre aspas** somado a `-Cd "<path>"` entre aspas (vários segmentos entre aspas no meio do comando) **quebra o match** da allowlist (que casa literal), além do **limite ~32KB de linha de comando do Windows** para prompts grandes; (2) **deriva de forma** — a mesma ação chamada de N jeitos (`& "<absoluto>"` com aspas vs. `pwsh -NoProfile -File scripts/<X>.ps1` relativo sem aspas), cada variante exigindo uma entrada de allowlist distinta. Caso real 2026-06-21: `Invoke-Codex.ps1` chamado como `& "<abs>\Invoke-Codex.ps1" "<prompt>" -Model gpt-5.5 -Cd "<abs>" -TimeoutSec 420` promptou apesar das entradas `PowerShell(& "…\scripts\*")` existentes.
- **Maturidade** — (a) IMPLEMENTADA 2026-06-22; resíduo (b) ainda aberto.
  - **(a) Dar `-MessagePath <arquivo>` aos adapters — IMPLEMENTADO** (2026-06-22): estendido a `Invoke-Codex.ps1`/`Start-CodexJob.ps1`/`Invoke-ClaudeCode.ps1`/`Start-ClaudeCodeJob.ps1` (stdin-based) e `Invoke-Gemini.ps1`/`Invoke-Copilot.ps1` (argument-based, com guard de tamanho), espelhando o `Invoke-OpenCode.ps1`. O despacho fica atômico (`pwsh -NoProfile -File scripts/Invoke-Codex.ps1 -MessagePath <arquivo> …`, zero aspas embutidas). Convergido por revisão por pares (v1→v4, 5 revisores/3 famílias). Migrado ao `historico/IdeiasImplementadas_202606.md`; ver `CHANGELOG` (Unreleased).
  - **(b) AINDA ABERTO — documentar UMA forma canônica de invocação** desses scripts na `xpz-llm-delegate/SKILL.md` e no `AGENTS.md` (repo serve vários agentes — Claude Code, Codex, Cursor, OpenCode —, então a convenção tem que morar onde **todo** agente lê, não na memória pessoal de um), e **enxugar a allowlist** para cobrir essa forma única em vez de N variantes.
- **NÃO** criar um wrapper guarda-chuva novo só para driblar o matcher: custa inventário (`09`), paridade e manutenção; o ganho real vem de (a)+(b). Um único `Invoke-PeerReviewer.ps1` seria só ergonomia opcional, não a raiz. Princípio: **menos variação > mais scripts**.
- **Relacionada:** absorve o eixo de atrito da entrada seguinte (migração stdin de `Invoke-Gemini`/`Invoke-Copilot`) para o caso `-MessagePath`; a parte de **stdin + limite de 32KB** dessas duas continua dependente de assinatura para validação empírica e fica naquela entrada.
...
**CONTESTAÇÃO + evidência (2026-06-21, mesmo dia — LER ANTES DE IMPLEMENTAR):** a tese "coder trunca por tool-calls" está **sob dúvida**. (1) Um agente de outra sessão **não reproduziu** o truncamento: chamando kimi/minimax via `Invoke-OpenCode.ps1 -Raw` (agente default), AMBOS executaram a rotina real e devolveram parecer final (`reason=stop` no ÚLTIMO `step_finish`; o `tool-calls` só no passo intermediário da bash). (2) Inspeção dos **logs do opencode** (`~/.local/share/opencode/log/`) desta sessão: os logs com HTTP **429**/"weekly usage limit" são **só** os do período de cota estourada (~20:47–20:52); os runs **pós-renovação** (quando minimax/kimi deram `truncada (reason=tool-calls)`) **não têm 429**. Ou seja: **não** foi cota mascarada nesses runs truncados, **mas** o truncamento **não é reprodutível**. (3) Confound: o sucesso "read-only" foi **pós-renovação** (cota OK em ambos), então não se atribui limpo ao prompt vs. não-determinismo. **Hipótese revisada:** não-determinismo do modelo ou **esgotamento do orçamento de passos** do opencode (coder gasta rodadas de tool-call e o turno acaba na tool-call) num workload pesado — **não** propriedade fixa "coder=trunca", **não** 429. **Reclassificação:** a variante read-only é, no máximo, **otimização de robustez** (menos rodadas de tool-call), **não** correção de mecanismo. **Antes de formalizar em `15`/`SKILL`**, confirmar com evidência crua (`-Raw`/JSONL) qual o último `step_finish` real e se há limite de passos do opencode. Limitação desta sessão: o JSONL cru **não** foi salvo (Invoke-OpenCode non-Raw limpa os temporários), só o veredito final do wrapper.

**RESOLUÇÃO (2026-06-22):** investigação concluída (8 chamadas reais `-Raw` nesta sessão). (1) A hipótese "esgotamento do orçamento de passos" foi **refutada** no cenário testado: kimi fez **7 tool-calls** e fechou no **8º passo** com `reason=stop` — o teto do opencode aguenta ≥8 (não prova ausência de teto em workloads maiores). (2) A truncagem é **real mas intermitente e rara**, não reproduzida; **não** é cota/timeout/orçamento-de-passos/propriedade-fixa → hipótese sobrevivente = **não-determinismo de cauda**. (3) **Correção escolhida e IMPLEMENTADA:** `-MaxAttempts` (retry-once opt-in) em `Invoke-OpenCode.ps1` — agnóstica à causa, converte a maioria das truncagens raras em sucesso (ver `CHANGELOG.md`, `xpz-llm-delegate/SKILL.md` junto ao Achado D, self-test `Test-OpenCodeRetrySelfTest.ps1`). (4) A "variante read-only" **não** se faz por prompt (`--agent plan` **auto-aprova bash em headless** — refutado empiricamente), e sim por **agente custom `permission: bash: deny`** — reenquadrada como a frente própria «agente reviewer sem execução/escrita» abaixo. Plano convergido por **revisão por pares** (v1→v5, 5 vozes / 3 famílias: anthropic nativo, openai/Codex gpt-5.5, ollama-cloud deepseek-v4-pro/glm-5.2/kimi-k2.7-code). Considerar migração ao histórico mensal.

## Painel de revisão por pares concede execução de código a modelo externo em headless (least-privilege do opencode)

- **Importância** — média (segurança / least-privilege).
...
## Implementar `Invoke-LlmDelegatePanelDispatch.ps1` (frente A) — IMPLEMENTADO (v11); falta push

**Status (2026-06-22):** **IMPLEMENTADO** conforme a v11. `scripts/Invoke-LlmDelegatePanelDispatch.ps1` + self-test `scripts/Test-InvokeLlmDelegatePanelDispatchSelfTest.ps1` (verde, sentinela `OK:`), parse 271/0, paridade documental aplicada (`09`, `xpz-llm-delegate/SKILL.md` SCRIPTS + seção «Harness de disparo do painel», `15:94` futuro→implementado, CHANGELOG trilíngue). **Falta:** revisão por pares **da implementação** + rotina pré-push + commit/push (sob autorização do usuário). Migração desta entrada ao histórico mensal fica para o encerramento.

Design **convergido por revisão por pares** — painel inteiro (3 famílias: anthropic via subagente nativo, openai/Codex `gpt-5.5`, ollama-cloud `deepseek-v4-pro`+`kimi-k2.7-code`) **4 APROVA / zero gaps** sobre a versão final **v11**, **incluindo o dissidente Codex** (que achou o gap P1 — circularidade do `$effectiveModel` do opencode — na v10 e **reaprovou** na v11). Closeout mecânico `closeoutReady=true`, `vNextState=resubmitted`. Trajetória v6→v11 (rodadas + 2 consultas de decisão; preferidos `claude-code` cobertos pelo nativo e `glm-5.2` `skippedByHumanDecision` por flakiness/2× timeout). **Spec congelada:** `Temp/revisao-por-pares/panel-v11/manuscrito.md` (ATENÇÃO: `Temp/` é **gitignored** — ler/versionar antes de limpar o scratch; o essencial está resumido abaixo).

**Importância:** média-alta — é o «harness de disparo do painel» previsto como futuro em `15-revisao-por-pares.md:94` (hoje a orquestração da revisão por pares é ad-hoc). **Maturidade:** implementado + self-test + paridade documental; falta o ciclo de fechamento (revisão por pares da implementação, pré-push, push).

**O que é:** harness **MECÂNICO** de despacho+coleta de um painel de revisores a backends LLM via os adapters `Invoke-*.ps1`; coleta vereditos no ledger e emite `panel-summary.json`. **NÃO** decide diversidade/piso/closeout/triagem/convergência/autorização — isso é do orquestrador.

**Contrato load-bearing (resumo da v11; detalhe e fontes no manuscrito):**
- **Assinatura:** `-ManuscriptPath -ReviewersJson -PayloadSensitivity <public|kb-sensitive> [-RoundId -Cd -ParallelKbRoot -PolicyPath -TempDir -OllamaConcurrency(ValidateRange 1..16, default 3)]` + TEST-ONLY `-OpenCodeConfigPath/-CodexConfigPath/-BackendExeMap`.
- **Entrada `ReviewersJson`:** `[{backend, targetModelKey, invokeArgs, family?}]` (o orquestrador decidiu o conjunto; subagente nativo injetado FORA do harness). `family` por prefixo do `targetModelKey` canônico; vazio→`$null` (não conta no piso).
- **Por revisor:** opencode+`kb-sensitive` → `unavailable` (sem gate/despacho; confinamento deferido). **Modelo efetivo:** opencode = `invokeArgs.model` ou o `targetModelKey` de **entrada** (não o do gate — o resolvedor opencode exige `-Model`); codex = `invokeArgs.model` ou, se ausente, gate **sem** `-Model` → deriva o **último segmento** do `targetModelKey` retornado; claude-code/copilot/gemini sem `model` → `state=error` **fail-closed**. Gate `Resolve-LlmDelegateAuthorization.ps1` (sem autorizar; codex repassa `oss/localProvider/profile`); `allow`→despacha, `ask/deny`→`gateAsk/gateDeny`.
- **Despacho concorrente:** `ForEach-Object -Parallel -ThrottleLimit 8` + `[SemaphoreSlim]::new($OllamaConcurrency)` via `$using:` **só** para `ollama-cloud/*` (validado empírico PS 7.6.2: não lança, limita a 3); capturar `$collected` **antes** do `Dispose`. `-Cd` precedência (explícito→`ParallelKbRoot` em sensível→cwd) + fail-closed; allowlist de splat per-backend; contenção (`permissionMode/tools/maxTurns/agent`, `approvalMode!=plan`) → `securityBlockedArgs` (trava fail-closed per-backend); `-MaxAttempts 2` só opencode.
- **Classificação (estrutural, SEM parsear prosa):** texto não-vazio→`responded` (**mecânico**; off-task→`noResponse` é reclassificação **post-hoc do orquestrador**, `15:53`); cota→`unavailable`; timeout→`timeout`; resto→`error`. **single-flight DEFERIDO** (decisão II-b) — falha concorrente→`error` + ledger cru + `concurrencySaturationWarning` em stderr; a recuperação automática depende do contrato de saída **tipado** dos adapters (frente «Detecção de truncamento…» acima).
- **Saída:** stdout = **1 linha JSON** `Kind=xpz-llm-panel-dispatch-result`/`SchemaVersion=1` (PascalCase; UTF-8 sem BOM; `[Console]::Error` para todo texto humano; o **chamador captura stdout/stderr separados**); ledger por estado (`.verdict.txt`/`.error.txt`/`.state.txt`) + `manifest.json`; estados emitidos ⊆ válidos do closeout (`Resolve-LlmDelegatePeerReviewCloseout.ps1:177-187`).

**Decisões de design registradas:** **I-a** (`ForEach-Object -Parallel`, NÃO `ThreadJob` — confirmado ausente na máquina); **II-b** (deferir single-flight); contenção **fora** da allowlist = **Posição B** (decisão de segurança, confirmada por consulta de 3 famílias); opencode **só-`public`** no v1 (confinamento por agente custom deferido); **NÃO** refatorar os 6 adapters agora (status tipado é frente própria).

**Relacionado:** `15-revisao-por-pares.md` (harness de disparo, `:94`); `xpz-llm-delegate/SKILL.md` (mecanismo + Achado B/contenção + cota semanal ollama + single-flight); `scripts/Resolve-LlmDelegate*.ps1` (gate/piso/closeout/curadoria) e `scripts/Invoke-*.ps1` (adapters); frentes «contrato de saída ESTRUTURADO dos adapters» (consumidor do single-flight) e «agente reviewer sem execução/escrita» (confinamento do opencode).
