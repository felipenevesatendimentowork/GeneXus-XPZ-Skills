...
9-WW. WorkWithWeb Apply-mark preflight gate — run before any packaging when the batch contains a `WorkWithForWeb` object (type `78cecefe-be7d-4980-86ce-8d6e91fba04b`; `Object/@name` starts with `WorkWithWeb` in the KB):
   - For each `WorkWithForWeb` in the batch, detect the structural form by Part type present:
     ...
     - **Form B (acervo form)**: Part `a51ced48-7bee-0001-ab12-04e9e32123d1` with `<Data Pattern="78cecefe-be7d-4980-86ce-8d6e91fba04b">` whose CDATA contains `<transaction transaction="<guid>-<name>" />` referencing the linked Transaction. Used in `ObjetosDaKbEmXml/WorkWithForWeb/*.xml` (materialized acervo).
     - If both Parts are present, prefer Form A (explicit `Apply` property is more informative); emit alert noting the unusual coexistence.
     - If neither Part is present, the XML is unrecognizable as a WorkWithForWeb in either expected form — **ABORT** with structural error.
   - In Form A: read `Apply` Property value:
     - Absent → **ABORT**: the `Apply` mark is missing; add `<Name>Apply</Name><Value>True</Value>` before packaging.
     - `False` → alert: pattern application is explicitly disabled; confirm whether this is intentional before continuing.
     - `True` → proceed to Transaction check; read linked Transaction name from `<Property><Name>Transaction</Name><Value>...`.
   - In Form B: treat `Apply` as implicitly `True` (a WorkWithForWeb materialized in the acervo was generated by the pattern; the `<Data Pattern="...">` element confirms the pattern is bound). Read linked Transaction name from `<transaction transaction="<guid>-<name>" />` inside the CDATA: the name follows the last `-` in the value (the format is `<guid>-<TransactionName>`).
   - With the linked Transaction name resolved (either form):
     - If that Transaction is present in the batch: read its `<Properties>` and check for `<Name>Apply:78cecefe-be7d-4980-86ce-8d6e91fba04b</Name><Value>True</Value>`.
       - Property absent or `False` → **ABORT**: Transaction `X` is in the batch but lacks the `Apply:78cecefe-be7d-4980-86ce-8d6e91fba04b = True` mark; the IDE will not re-apply the pattern when saving that Transaction; add the property before packaging.
     - If that Transaction is in `ObjetosDaKbEmXml` but not in the batch: read the corpus XML and check for the same `Apply:78cecefe-be7d-4980-86ce-8d6e91fba04b = True` property.
       - Property absent or `False` → alert: Transaction `X` exists in the official corpus but lacks the `Apply:GUID = True` mark; verify whether the existing KB state will preserve the expected pattern behavior after import.
     - If the linked Transaction is absent from both → alert: cannot confirm the `Apply:GUID` mark; document the gap before packaging.
