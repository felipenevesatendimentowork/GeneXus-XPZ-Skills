...
- [ ] When `New-XpzImportPackage.ps1`/`.py` was used with `-AcervoPath`, the 9-FD front-acervo drift gate (`Test-GeneXusFrontAcervoDrift.ps1`) ran before packaging and returned `pass` or all findings were resolved (for `fail` findings, packaging was blocked; for `warn` findings — `front-equals-acervo` or `lastupdate-unparseable` — each was explicitly acknowledged or resolved before continuing)
- [ ] When the 9-FD drift gate reported `front-older-than-acervo`, resolution used `Copy-GeneXusAcervoToFront.ps1` or manual copy-and-bump, followed by re-running the 9-FD gate to confirm the drift was cleared; `ObjetosDaKbEmXml` was never edited in place to "fix" drift (anti-pattern "editar acervo esperando que o pacote pegue")
- [ ] When an existing corpus object needed initial seed into the front, `Copy-GeneXusAcervoToFront.ps1` was called with explicit `-ObjectNames` or `-ObjectGuids`; no implicit seed of a whole empty front was assumed.
