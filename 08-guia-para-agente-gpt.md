...
- na rotina pre-push, executar primeiro `scripts/Invoke-PrePushMechanicalChecks.ps1` em `pwsh` 7.4+ (`-AsJson` recomendado para agentes); por padrao compara com `origin/main` — commits locais ainda nao enviados ao remoto — usando o mesmo intervalo para contagem, lista de commits, arquivos alterados e `git diff --check`; o script avisa se branch != main ou working tree suja/untracked (fora do intervalo de commits), sem falhar o gate mecanico
- o script mecanico nao encerra a pre-push; em seguida aplicar a busca semantica de coerencia cruzada descrita no `AGENTS.md` da raiz, incluindo a regra em camadas para `SKILL.md` longos (checklist final, captura de resultado e inventario de scripts no mesmo arquivo)
- na pre-push, gaps exigem aprovacao explicita depois do relatorio; intencao inicial de aplicar correcoes nao dispensa esse passo nem autoriza gravar automaticamente
