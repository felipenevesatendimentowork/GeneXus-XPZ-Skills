...
- em wrappers MSBuild que suportam watcher (`Invoke-GeneXusKbBuildAll.ps1`, `Invoke-GeneXusKbSpecifyGenerate.ps1`, `Test-GeneXusXpzImportPreview.ps1`, `Invoke-GeneXusXpzImport.ps1`, `Invoke-GeneXusXpzExport.ps1`), usar `-StartWatcher` com `-MonitorLogPath` como padrao em execucao longa ou import amplo/com muitos `WorkWithForWeb`; execucao sem watcher visivel exige justificativa operacional explicita; se `watcherContext.watcherLaunched=false`, declarar a ausencia ao usuario
