...
- o parse delegado (`Test-PsScriptsParse.ps1` e `Test-PyScriptsParse.ps1`) varre todo `scripts/` aplicavel, nao apenas o diff; com `commitsAhead=0` nao ha diff no intervalo — alteracoes so na working tree ficam fora da analise (o script avisa contagens)
