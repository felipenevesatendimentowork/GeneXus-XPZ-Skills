...
- em KB com `kb_environment_count` > 1 em `kb-source-metadata.md`, build de validacao pós-import exige `-ParallelKbRoot` (ou `-KbMetadataPath`), `-EnvironmentName` ou `deployment_environment_name` gravado pelo `xpz-kb-parallel-setup`; nao confundir `compilou limpo` no environment ativo da IDE com deploy no IIS; fix completo multi-environment exige rodada por nome em `kb_environment_names` (`02`, `xpz-msbuild-build`)
