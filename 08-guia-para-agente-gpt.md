...
- em `Invoke-GeneXusKbBuildAll.ps1` e `Invoke-GeneXusKbSpecifyGenerate.ps1`, quando `stdoutSignals.postBuildEvents` vier com itens, tratar como evento pos-build configurado na KB que disparou ou tentou disparar processo externo; esses itens sao capturados preferencialmente pela janela `Executando eventos pos-construcao ...` ate o proximo separador `==========`, com fallback para `start c:` / `start cmd` em logs sem marcador; nao declarar `compilou limpo` nem `specify e generate concluidos` como classificacao limpa nesse caso — usar `operacao concluida, pendente de confirmacao funcional` e citar o primeiro evento ou orientar inspeção de `stdoutSignals.postBuildEvents`
