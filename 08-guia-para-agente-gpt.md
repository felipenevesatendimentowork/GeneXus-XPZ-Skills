...
- antes de `who-uses`, `what-uses`, `impact-basic` ou `functional-trace-basic`, conferir em `scripts/gx-object-type-catalog.json` se o tipo tem `queryableByKbIntelligence=true`; quando for `false` (ex.: `SmartDevicesPlus`), o `Query-KbIntelligenceIndex` devolve `blocked=true` e exit `11` — **nao** tratar como “zero dependencias”; usar `object-info`, `search-objects`, `list-by-type` ou XML pontual
