...
- delegar tarefa menor ou pedir segunda opiniao a um modelo secundario (skill `xpz-llm-delegate`, backends opencode, Codex — `codex exec` —, Claude Code — `claude -p`, Opus 4.8 —, GitHub Copilot CLI — `copilot -p` — e Gemini CLI — `gemini -p`) só **a pedido do usuário ou com concordancia explicita dele**; nunca por iniciativa própria
