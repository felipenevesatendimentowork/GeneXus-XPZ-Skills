...
- apos export/import/preview MSBuild: distinguir **Categoria A** (extras de inventario, modulos/ExternalObjects de plataforma, `attributesTopLevelUnreconciled` — `exitCode=0` possivel, ABORT do agente via Decisao pos-gates) de **Categoria B** (linhas `error :`, `invalidTypesRejected`, `importErrors`/`previewErrors` — wrapper rebaixa para **`exitCode=48`**, `msBuildCategoryBBlocked=true`); ver secao «Categorias A e B» em `xpz-msbuild-import-export/SKILL.md` e `scripts/msbuild-exit-codes.catalog.json`
