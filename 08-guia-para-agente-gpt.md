...
- na rotina pre-push, executar primeiro `scripts/Invoke-PrePushMechanicalChecks.ps1` em `pwsh` 7.4+ (`-AsJson` recomendado para agentes); intervalo unico `origin/main..HEAD` para contagem, lista, arquivos do diff e `git diff --check`; parse via `Test-PsScriptsParse.ps1` varre todo `scripts/` e `*.example.ps1` (fora de `historico/`), nao so o diff; com `commitsAhead=0` nao ha diff no intervalo — pre-push nao revisa so working tree (so avisa); ler `PUSH_READINESS`, nao so exit 0; `blocked` nao dispensa fase semantica
