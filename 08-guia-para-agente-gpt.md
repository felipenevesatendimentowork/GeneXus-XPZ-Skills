...
- apos export headless com XPZ gerado: ler `packageInventory` e `operationalSubState` no `export.json` (e `package-inventory.json` no diretório de artefatos) antes de declarar conclusao limpa; **nunca** reportar ao usuario a contagem de `-ObjectList` como contagem do pacote — reproduzir totais reais (`totalObjects`, `totalAttributes`, `objectsByType`, `systemModulesPresent`)
