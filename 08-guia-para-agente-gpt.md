...
- apos preview ou importacao MSBuild, preferir `msbuild.import.signals.json` ao lado dos logs brutos (gerado por `Read-MsBuildImportSignals.ps1` via `Test-GeneXusXpzImportPreview.ps1` ou `Invoke-GeneXusXpzImport.ps1`) para leitura compacta de itens, warnings, erros, `gxImportLogReadStatus`/`gxImportLogReadError` e campos canonicos; so reparsear `msbuild.stdout.log`/`msbuild.stderr.log` inteiros quando esse artefato estiver ausente ou insuficiente
