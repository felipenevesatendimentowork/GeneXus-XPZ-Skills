...
- apos export headless com XPZ gerado: ler `packageInventory`, `operationalSubState`, `exportErrors`, `invalidTypesRejected` e `knownStdOutNoise` no **top-level** do `export.json` (e `package-inventory.json` via `nominalInventoryAt` / `packageInventoryPath` / `artifacts.PackageInventoryPath`, e `msbuild.export.signals.json` no diretorio de artefatos) antes de declarar conclusao limpa; se `operationalSubState` for `exportação concluída com erros parciais ignorados pela task`, reproduzir as linhas `error :` ao usuario mesmo com `exitCode=0`; declarar `knownStdOutNoise` (ex. `cssproperties-access-denied`) quando nao vazio — nao depender so de `stdoutSignals`; **nunca** reportar ao usuario a contagem de `-ObjectList` como contagem do pacote — reproduzir totais reais e, quando `extrasCount > 0`, listar extras por nome; quando `attributesTopLevelUnreconciled=true`, listar atributos top-level por nome (com `Transaction` na lista, enumera nominal de atributos so se o usuario pedir); em perguntas sobre conteudo nominal do XPZ (mesma rodada ou sessao nova), usar sidecar quando existir (caminho A) ou `Get-GeneXusImportPackageObjectInventory.ps1 -InputPath <xpz> -AsJson` (caminho B) antes de responder
