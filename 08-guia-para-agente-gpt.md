...
- apos export headless com XPZ gerado: ler `packageInventory`, `operationalSubState`, `exportErrors`, `invalidTypesRejected` e `knownStdOutNoise` no **top-level** do `export.json` (e `package-inventory.json` / `msbuild.export.signals.json` no diretório de artefatos) antes de declarar conclusao limpa; se `operationalSubState` for `exportação concluída com erros parciais ignorados pela task`, reproduzir as linhas `error :` ao usuario mesmo com `exitCode=0`; declarar `knownStdOutNoise` (ex. `cssproperties-access-denied`) quando nao vazio — nao depender so de `stdoutSignals`; **nunca** reportar ao usuario a contagem de `-ObjectList` como contagem do pacote — reproduzir totais reais (`totalObjects`, `totalAttributes`, `objectsByType`, `systemModulesPresent`, `systemExternalObjectsPresent`, `attributesTopLevelUnreconciled` e `inventoryWarnings` quando existirem)
