...
- na rotina pre-push, executar primeiro `scripts/Invoke-PrePushMechanicalChecks.ps1` em `pwsh` 7.4+ (`-AsJson` recomendado para agentes); por padrao compara com `origin/main` — commits locais ainda nao enviados ao remoto — usando o mesmo intervalo para contagem, lista de commits, arquivos alterados e `git diff --check`
