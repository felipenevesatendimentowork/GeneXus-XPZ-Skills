...
- na rotina pre-push, executar primeiro `scripts/Invoke-PrePushMechanicalChecks.ps1` (`-AsJson` recomendado para agentes): por padrao compara com `origin/main` (commits locais ainda nao enviados ao remoto; contagem, lista de commits, arquivos alterados e `git diff --check` usam o mesmo intervalo); em seguida aplicar a busca semantica de coerencia cruzada descrita no `AGENTS.md` da raiz — o script mecanico nao encerra a pre-push; inclui regra em camadas para skills longas (checklist, captura de resultado e inventario de scripts no mesmo `SKILL.md`); exemplos canonicos ficam em `*.example.ps1` dentro das skills afetadas (hoje principalmente `xpz-kb-parallel-setup/examples/`), nao em `examples/` na raiz
