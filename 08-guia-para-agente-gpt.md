...
- antes de importacao real: alem do gate de envelope (`Test-GeneXusImportFileEnvelope.ps1`), **inventariar todos os objetos** do pacote e conciliar com o delta declarado; preferir `Get-GeneXusImportPackageObjectInventory.ps1 -InputPath <import_file.xml ou .xpz> -AsJson` e, se houver lista esperada, `-DeclaredDeltaPath` ou `-DeclaredDeltaItems`; em export seletiva sem `Transaction` na lista, atributos top-level em massa (`attributesTopLevelUnreconciled`) indicam arrasto de base — declarar antes de import/sync; pacotes vindos de **export MSBuild** podem trazer objetos extra nao pedidos — nao assumir que tudo no zip era intencional
