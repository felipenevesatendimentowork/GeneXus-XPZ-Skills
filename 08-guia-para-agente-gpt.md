...
- na rotina pre-push, executar primeiro `scripts/Invoke-PrePushMechanicalChecks.ps1` (`-AsJson` recomendado para agentes) para contexto git, `git diff --check`, classificacao dos arquivos alterados e delegacao de parse a `scripts/Test-PsScriptsParse.ps1`; em seguida aplicar a busca semantica de coerencia cruzada descrita no `AGENTS.md` da raiz, incluindo a regra em camadas para skills longas (checklist, captura de resultado e inventario de scripts no mesmo `SKILL.md`); exemplos canonicos ficam em `*.example.ps1` dentro das skills afetadas (hoje principalmente `xpz-kb-parallel-setup/examples/`), nao em `examples/` na raiz
