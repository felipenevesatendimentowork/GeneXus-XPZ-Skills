...
- em wrappers MSBuild que suportam watcher (`Invoke-GeneXusKbBuildAll.ps1`, `Invoke-GeneXusKbSpecifyGenerate.ps1`, `Test-GeneXusXpzImportPreview.ps1`, `Invoke-GeneXusXpzImport.ps1`, `Invoke-GeneXusXpzExport.ps1`), usar `-StartWatcher` com `-MonitorLogPath` como fluxo padrao em `BuildAll` e `SpecifyGenerate`, e como padrao para preview/export/import longos; em importacao real de pacote amplo ou com muitos `WorkWithForWeb`, a ausencia de watcher exige justificativa operacional explicita; o contrato comum fica centralizado em `scripts/GeneXusMsBuildWatcherSupport.ps1`; se `watcherContext.watcherLaunched=false`, declarar a ausencia ao usuario
