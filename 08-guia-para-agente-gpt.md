...
- em export seletivo: passar `-ParallelKbRoot` (raiz da pasta paralela da sessao) ou `-IndexPath` ao `Invoke-GeneXusXpzExport.ps1`; homonimia ou indice ausente/invalido → **exit 35**; item ausente no indice (`not_in_index`) → aviso no `export.json`, MSBuild pode seguir (objeto novo so na KB nativa); inventario pos-export continua obrigatorio
