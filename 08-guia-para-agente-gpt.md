...
- no diagnostico JSON dos wrappers, `exitCode` e o valor classificado pelo wrapper (0/32/41/42/...) e tambem o exit code do processo; `msBuildExitCode` e o valor bruto da task MSBuild, preservado em campo proprio; `executionEvidence` concentra evidencia bruta de execucao e logs; `blockingReasons` deve ser lido como lista de causas acionaveis, nao como espelho do exit bruto
