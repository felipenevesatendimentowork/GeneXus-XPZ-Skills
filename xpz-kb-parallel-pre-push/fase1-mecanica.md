...
| **K1/K2** paths perigosos | `block` · `unknown` | `Test-XpzKbDangerousPaths.ps1` | motor com `status=unknown` (ex.: git falho) → gate `unknown` (fail-open: lê o status top-level para não sumir da consolidação) |
