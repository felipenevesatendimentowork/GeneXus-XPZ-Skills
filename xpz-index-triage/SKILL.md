...
- Antes de `who-uses`, `what-uses`, `impact-basic` ou `functional-trace-basic`, conferir no catálogo efetivo (`scripts/gx-object-type-catalog.json` + override local) se o tipo tem `queryableByKbIntelligence=true`; quando for `false` (ex.: `DataStore`, `Generator`, `ThemeClass`, `PatternSettings`, `SmartDevicesPlus`), o query devolve exit `11` e `blocked=true` — **não** tratar como “zero dependências”; usar `object-info`, `search-objects`, `list-by-type` ou XML pontual.
