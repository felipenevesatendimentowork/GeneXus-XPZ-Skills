...
- Tratar `kb-source-metadata.md` por autoridade de campo, nao por dono unico do arquivo:
  ...
  - environments GeneXus (`deployment_environment_name`, `kb_environment_count`, `kb_environment_names`): autoridade desta skill via `scripts/Set-XpzKbSourceMetadataDeployment.ps1` (wrapper local `Set-*KbSourceMetadataDeployment.ps1`); inventario de pastas na KB nativa (`<KbNativa>\<Env>\web\`) ocorre **somente** nessa rotina quando `-InventoryFromKbNativePath` for usado — **nunca** em cada build/import MSBuild
