...
- Tratar `kb-source-metadata.md` por autoridade de campo, nao por dono unico do arquivo:
  ...
  - environments GeneXus (`deployment_environment_name`, `deployment_hosting_kind`, `kb_environment_count`, `kb_environment_names`): autoridade desta skill via `scripts/Set-XpzKbSourceMetadataDeployment.ps1` (wrapper local `Set-*KbSourceMetadataDeployment.ps1`); inventario **obrigatorio no setup** via `-InventoryFromGeneXusMsBuild` (`scripts/Get-GeneXusKbEnvironmentNames.ps1` / `GeneXusKbEnvironmentInventorySupport.ps1`) — pre-filtro de pastas legadas na KB nativa + validacao `SetActiveEnvironment`; **nunca** `-InventoryFromKbNativePath` (heuristica `web\` removida: incluia `CSharpModel`, `Data*` e backups); build/import so leem metadata gravado
...
- Em `modo_atualizacao` ou apos confirmar caminho da KB nativa, executar `Set-XpzKbSourceMetadataDeployment.ps1` (ou wrapper local) com `-DeploymentEnvironmentName`, `-DeploymentHostingKind`, `-InventoryFromGeneXusMsBuild`, `-KbNativePath <caminho>` e `-InventoryWorkingDirectory <pasta-temp-segura>` (probe MSBuild); registrar os quatro campos no frontmatter de `kb-source-metadata.md`. `-KbEnvironmentNames` explicito so como excecao documentada quando MSBuild indisponivel no setup
