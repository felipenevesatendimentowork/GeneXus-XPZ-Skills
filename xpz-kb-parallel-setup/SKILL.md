...
- NUNCA propor alterar campos existentes de `kb-source-metadata.md` em `modo_atualizacao`, exceto gravar `last_setup_audit_run_at` ao declarar estado canonico de conclusao bem-sucedido (passo 34 do WORKFLOW); quando um wrapper local retornar campos errados ou ausentes, corrigir o wrapper para ler o formato real do arquivo — nunca alterar o arquivo de metadados para atender ao wrapper defasado.
