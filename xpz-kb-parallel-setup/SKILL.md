...
Quando `auditar_setup` detectar `metadata wrapper: PENDENTE_DE_DADOS`, `metadata wrapper: PENDENTE` ou `metadata wrapper: BLOCK`, ou `metadata/deploy: PENDENTE` ou `metadata/deploy: BLOCK`, nao declarar `materializado_e_indice_validado` nem gravar `last_setup_audit_run_at` como conclusao bem-sucedida. Se o proprio `estado_operacional_sugerido` ainda vier limpo nesse cenario, tratar como divergencia do motor de auditoria e corrigir a metodologia antes de usar o resultado para liberar a pasta.
