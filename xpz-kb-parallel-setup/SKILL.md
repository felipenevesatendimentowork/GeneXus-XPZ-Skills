...
Quando `estado_operacional_sugerido` for `atualizacao_metodologica_pendente`: ler a linha `wrappers/inventario:` da saida para identificar quais scripts estao ausentes; apresentar ao usuario, em portugues e sem termos tecnicos em ingles, quais scripts ainda nao existem na pasta (ex: "Os seguintes scripts ainda nao existem nesta pasta: [lista]"); perguntar se deseja criá-los agora antes de prosseguir com a tarefa original — se sim, executar `atualizar_bootstrap_local`; se nao, prosseguir com a tarefa original registrando o gap. Nao acionar o WORKFLOW de criacao/documentacao (passos 1-7b) — esse WORKFLOW e reservado para quando o usuario pede explicitamente setup, atualizacao ou auditoria.
...
8.d Nao tocar `kb-source-metadata.md` em modo_atualizacao, exceto para gravar `last_setup_audit_run_at` nos casos permitidos por esta skill; o arquivo contem timestamps operacionais reais que o gate de frescor depende e nao devem ser sobrescritos pelo agente. Essa excecao cobre tanto o passo 34 do WORKFLOW quanto o subestado `setup_apto_com_metadata_pendente` da PRE-CONDICAO, sempre preservando o restante do arquivo e respeitando regras locais de aprovacao antes da escrita.
...
8.g6.iii Se houver evidencia objetiva, esta skill nao audita o conteudo nem o comportamento de `Invoke-GeneXusXpzImport.ps1`. Esse script e do motor compartilhado e seu contrato (parametros, diagnostico JSON, resiliencia do pos-processamento, sub-estados de classificacao) e governado pela skill `xpz-msbuild-import-export`. A acao desta skill e:
    - declarar `importacao_msbuild = ADOTADO` no handoff e roteamento explicito para `xpz-msbuild-import-export` quando houver qualquer suspeita de diagnostico degradado (ex: `import.json` ausente, vazio, com `postProcessingFailed=true`, ou ausencia de `importedItems` apesar de `__IMPORTED_ITEM__` no log bruto)
...
- NUNCA gravar em `kb-source-metadata.md` se o arquivo ja existir com timestamps reais, exceto para gravar `last_setup_audit_run_at` ao declarar estado canonico de conclusao bem-sucedido (passo 34 do WORKFLOW) ou ao tratar `setup_apto_com_metadata_pendente` na PRE-CONDICAO; apos a primeira materializacao oficial esse arquivo e somente leitura para o agente para todos os demais campos
...
- NUNCA propor alterar campos existentes de `kb-source-metadata.md` em `modo_atualizacao`, exceto gravar `last_setup_audit_run_at` ao declarar estado canonico de conclusao bem-sucedido (passo 34 do WORKFLOW) ou ao tratar `setup_apto_com_metadata_pendente` na PRE-CONDICAO; quando um wrapper local retornar campos errados ou ausentes, corrigir o wrapper para ler o formato real do arquivo — nunca alterar o arquivo de metadados para atender ao wrapper defasado.
