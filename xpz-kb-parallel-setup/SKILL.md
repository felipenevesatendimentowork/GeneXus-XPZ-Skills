...
- Tratar `kb-source-metadata.md` por autoridade de campo, nao por dono unico do arquivo:
  ...
  - environments GeneXus (`deployment_environment_name`, `deployment_hosting_kind`, `kb_environment_count`, `kb_environment_names`): autoridade desta skill via `scripts/Set-XpzKbSourceMetadataDeployment.ps1` (wrapper local `Set-*KbSourceMetadataDeployment.ps1`); inventario de pastas na KB nativa (`<KbNativa>\<Env>\web\`) ocorre **somente** nessa rotina quando `-InventoryFromKbNativePath` for usado — **nunca** em cada build/import MSBuild
...
- Tratar `deployment_hosting_kind` como `dotnet-core-self-host` (sentinela `GxNetCoreStartup.dll` em `web\bin`) ou `dotnet-framework-iis` (checagem agregada de DLLs em `web\bin`) — obrigatorio para gate pos-import de `web\bin` (`xpz-msbuild-build`, exit **49**)
...
- Em `modo_atualizacao` ou apos confirmar caminho da KB nativa, executar `Set-XpzKbSourceMetadataDeployment.ps1` (ou wrapper local) com `-DeploymentEnvironmentName` e `-InventoryFromKbNativePath -KbNativePath <caminho>` **ou** `-KbEnvironmentNames` informados pelo usuario; registrar os tres campos no frontmatter de `kb-source-metadata.md`
