...
Quando a saida trouxer `metadata/deploy:` diferente de `OK` (`PENDENTE` ou `BLOCK`), tratar como pendencia metodologica da mesma severidade: `metadata wrapper: OK` nao prova plausibilidade semantica de `kb_environment_names`. O agente deve evidenciar `metadata/deploy.evidencia`, distinguir campos de deploy ausentes (`PENDENTE`) de metadata legado ou inconsistente (`BLOCK`), perguntar ao usuario os nomes exatos dos environments, validar cada nome via MSBuild (`SetActiveEnvironment` headless) e rerodar `Set-*KbSourceMetadataDeployment.ps1` com `-KbEnvironmentNames` antes de declarar estado limpo.
...
| `metadata/deploy` ≠ OK | Plausibilidade de environment/deploy em `kb-source-metadata.md` | Perguntar nomes ao usuario, rerodar `Set-*KbSourceMetadataDeployment.ps1` com `-KbEnvironmentNames` + validacao MSBuild; corrigir wrapper se `uses_removed_inventory_discovery` | Sim |
...
8. Gravacao de environment/deploy (`Set-*KbSourceMetadataDeployment.ps1` com `-KbEnvironmentNames` confirmado pelo usuario + validacao MSBuild obrigatoria) e correcao do wrapper local quando `metadata/deploy` ou `uses_removed_inventory_discovery` exigirem.
...
- Tratar `kb-source-metadata.md` por autoridade de campo, nao por dono unico do arquivo:
  ...
  - environments GeneXus (`deployment_environment_name`, `deployment_hosting_kind`, `kb_environment_count`, `kb_environment_names`): autoridade **declarada pelo usuario** (nomes exatos como na IDE) via `scripts/Set-XpzKbSourceMetadataDeployment.ps1` (wrapper local `Set-*KbSourceMetadataDeployment.ps1`); **obrigatorio** `-KbEnvironmentNames` + validacao MSBuild (`SetActiveEnvironment` headless sobre a lista informada); scan/inventario automatico por pastas da KB nativa (`-InventoryFromKbNativePath`, `-InventoryFromGeneXusMsBuild`) **removido**; `-SkipEnvironmentNamesMsBuildValidation` so quando sondagem MSBuild indisponivel por infraestrutura; build/import so leem metadata gravado
...
- Quando a pasta paralela da KB adotar `KbIntelligence`, a pasta `scripts` tambem deve prever wrappers locais finos para:
  ...
  - validacao de plausibilidade semantica de environment/deploy (`Test-XpzKbDeploymentMetadata.ps1`, consolidada em `Test-*KbSetupAudit.ps1` como `metadata/deploy`): rejeita metadata legado (tipico de scan por pastas `web\`) e inconsistencias de contagem; **nao** substitui pergunta ao usuario nem validacao MSBuild de nomes declarados
...
- NUNCA ignorar `metadata/deploy: BLOCK` ou `metadata/deploy: PENDENTE` na saida de `Test-*KbSetupAudit.ps1`; incluir `metadata/deploy.evidencia` no handoff, perguntar nomes ao usuario e rerodar `Set-*KbSourceMetadataDeployment.ps1` com `-KbEnvironmentNames` + validacao MSBuild antes de declarar estado limpo.
