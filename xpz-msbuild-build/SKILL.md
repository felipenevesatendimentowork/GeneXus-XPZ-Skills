...
9. Ler `exitCode`, `msBuildCategoryBBlocked` e `buildErrors`/`specifyErrors` no JSON **antes** de classificar. Com `exitCode=48` ou `msBuildCategoryBBlocked=true`, classificar como `falha operacional com rejeicao MSBuild no log`, reproduzir linhas `error :` ao usuário e **não** usar `compilou limpo` nem `specify e generate concluídos`. Caso contrário, escanear stdout e stderr por padrões de erro e risco antes de classificar, inclusive quando `executionEvidence.msBuildExitCode=0` e `exitCode=0`:
   ...
   - eventos pós-build: linhas dentro da janela `Executando eventos pos-construcao ...` ate o proximo separador `==========` em stdout; se o marcador nao existir, fallback para linhas `start c:` ou `start cmd` → registrar como warning de processos externos disparados
...
- [ ] Validação deploy pós-import usou `-PostImportDeployValidation` (ou `-StrictDeployBinCheck`) com `deployment_hosting_kind` no metadata; `deployBinFreshness`/`deployBinCheck`/`publicationFreshSinceBuild` foram lidos — **não** declarar deploy OK com `compilou-mas-dll-destino-desatualizada` ou exit **49**; warning de sentinela Core (`GxNetCoreStartup.dll` velho) não substitui gate
...
- NEVER declarar validação deploy OK quando `status` for `compilou-mas-dll-destino-desatualizada`, `deployBinFreshness=stale`, `publicationFreshSinceBuild=false` ou `exitCode=49` — investigar `deployBinCheck.interpretation`, `objectDllMaxWriteTime`/`configMaxWriteTime` e paths em `<KbNative>\<EnvDeploy>\web\bin`; **não** usar `GxNetCoreStartup.dll` sozinha como prova de publicação
